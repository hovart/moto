		...
		{foreach from=$products item=product name=products}
			...
			<li class="ajax_block_product {if $smarty.foreach.products.first}first_item{elseif $smarty.foreach.products.last}last_item{/if} {if $smarty.foreach.products.index % 2}alternate_item{else}item{/if} clearfix">
				...
				<div class="center_block">
					<a href="{$product.link|escape:'htmlall':'UTF-8'}" class="product_img_link" title="{$product.name|escape:'htmlall':'UTF-8'}"><img src="{$link->getImageLink($product.link_rewrite, $product.id_image, $tailleimg)}" alt="{$product.legend|escape:'htmlall':'UTF-8'}" /></a>
					{if $product.typesale==1 || $product.typesale==2 || $product.typesale==3}
