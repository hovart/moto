...
class MGDTOPLINK extends Module
{
	public function __construct()
	{
		$this->name = 'mgdtoplink';
		$this->description = 'Affichage des avantages concurrentiels dans la barre de nav';
		$this->tab = 'front_office_features';
		$this->version = '1.1.1';
		$this->author = 'emotionLoop';
		$this->need_instance = 0;
		$this->ps_versions_compliancy = array('min' => '1.5', 'max' => '1.6');
		$this->monolanguage_content = 'Mgdtoplink_MONOLANGUAGE';
		$this->text_editor_content = 'Mgdtoplink_TEXTEDITOR';
		$this->content_wrapper = 'Mgdtoplink_CONTENTWRAPPER';
		$this->content_wrapper_class = 'Mgdtoplink_CONTENTWRAPPER_CLASS';
		$this->content_wrapper_id = 'Mgdtoplink_CONTENTWRAPPER_ID';
		$this->bootstrap = true;
		$this->_html = '';
		...
		$this->displayName = $this->l('mgdTopLink');
		$this->description = $this->l('Affichage des avantages concurrentiels dans la barre de nav');
	...
	public function install()
	{
		...
		if (!parent::install() || !$this->registerHook('header')  || !$this->registerHook('footer') || !MGDTOPLINKModel::createTables())
			return false;
	...
	public function addFilesToTemplate($path = null)
	{
		...
		$files_data = MGDTOPLINKModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id );
	...
	{
		$content_query = MGDTOPLINKModel::getContent( $this->selected_store_id, $this->selected_language_id );
		$pre_content = '';
		$pos_content = '';
		...
		if (Tools::isSubmit('submit'.$this->name))
		{
				...
				&& (int)Configuration::get('PS_LANG_DEFAULT') == $this->selected_language_id ))
			{
				...
				if (Tools::getValue('content_text') !== false && $this->ignore_changes_content_changes == false)
					MGDTOPLINKModel::setContent( Tools::getValue('content_text'), $this->selected_store_id, $this->selected_language_id );

				//store the files to be used
				if (Tools::getValue('headerFiles') !== false)
					MGDTOPLINKModel::setFiles(
	...
	public function displayForm()
	{
		...
		$helper->fields_value['headerFiles[]'] = $this->processSelectFilesForMultiselect(
																MGDTOPLINKModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id )
																);
...
class MGDTOPLINKModel extends ObjectModel
{
	...
	public static function createTables()
	{
		...
		return ( MGDTOPLINKModel::createFilesTable()
				&& MGDTOPLINKModel::createContentTable());
	}
