...
{if !empty($bDisplayReviews) && !empty($aReviews)}
	...
	<div class="block last-reviews-{$sPosition|escape:'htmlall':'UTF-8'}" {if !empty($aBadeOptions.width) || !empty($aBadeOptions.height)}style="{if !empty($aBadeOptions.width)}width: {$aBadeOptions.width|intval}% !important;{/if}"{/if}>
		<p class="title_block">
			<a href="{$sReviewsControllerUrl|escape:'htmlall':'UTF-8'}" title="{l s='all Last reviews' mod='gsnippetsreviews'}">
				{l s='Last reviews' mod='gsnippetsreviews'}
			</a>
		</p>
