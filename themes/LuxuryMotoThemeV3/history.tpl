					...
					<tr class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if} {if $smarty.foreach.myLoop.index % 2}alternate_item{/if}">
						...
						<td{if isset($order.order_state)} data-value="{$order.id_order_state}"{/if} class="history_state">
							{if isset($order.order_state)}
								<span class="label{if isset($order.order_state_color) && Tools::getBrightness($order.order_state_color) > 128} dark{/if}"{if isset($order.order_state_color) && $order.order_state_color} style="background-color:{$order.order_state_color|escape:'html':'UTF-8'}; color:#fff; padding:5px;"{/if}>
						...
						<td class="history_invoice">
							{if (isset($order.invoice) && $order.invoice && isset($order.invoice_number) && $order.invoice_number) && isset($invoiceAllowed) && $invoiceAllowed == true}
								<a class="link-button" href="{$link->getPageLink('pdf-invoice', true, NULL, "id_order={$order.id_order}")|escape:'html':'UTF-8'}" title="{l s='Invoice'}" target="_blank">
									<i class="fa fa-file-text left"></i>{l s='PDF'}
								</a>
							{else}
								-
							{/if}
						...
						<td class="history_detail">
							<a class="button" href="javascript:showOrder(1, {$order.id_order|intval}, '{$link->getPageLink('order-detail', true)|escape:'html':'UTF-8'}');">
								<i class="fa fa-plus left"></i>{l s='Details'}
							</a>
							{if isset($opc) && $opc}
							...
							{if isset($reorderingAllowed) && $reorderingAllowed}
								<i class="fa fa-refresh left"></i>{l s='Reorder'}
							{/if}
							</a>
						</td>
					</tr>
...
<ul class="footer_links clearfix">
	<li>
		<a class="button std-btn" href="{$link->getPageLink('my-account', true)|escape:'html':'UTF-8'}">
			<i class="fa fa-chevron-left left"></i> {l s='Back to Your Account'}
		</a>
	</li>
	<li>
		<a class="button std-btn" href="{$base_dir}">
			<i class="fa fa-chevron-left left"></i> {l s='Home'}
		</a>
	</li>
</ul>