...
<div id="ap5-page-product-box">
	<h3 class="page-product-heading" id="#ap5-product-footer-pack-list"><span>{l s='This product is also available in pack' mod='pm_advancedpack'}</span></h3>
	...
	{foreach from=$packList item=productsPack key=idPack}
		...
		{if !$priceDisplay || $priceDisplay == 2}
			{assign var='productPrice' value=AdvancedPack::getPackPrice($idPack, true, true, true, 6, array(), array(), true)}
			{assign var='productPriceWithoutReduction' value=AdvancedPack::getPackPrice($idPack, true, false, true, 6, array(), array(), true)}
		{elseif $priceDisplay == 1}
			{assign var='productPrice' value=AdvancedPack::getPackPrice($idPack, false, true, true, 6, array(), array(), true)}
			{assign var='productPriceWithoutReduction' value=AdvancedPack::getPackPrice($idPack, false, false, true, 6, array(), array(), true)}
		{/if}
		<div id="ap5-product-footer-pack-container-{$idPack|intval}" class="ap5-product-footer-pack-container">
			<h5 class="ap5-product-footer-pack-name product-name">
				<a href="{$link->getProductLink($productsPack.packObj)|escape:'html':'UTF-8'}" title="{$productsPack.packObj->name|escape:'html':'UTF-8'}" itemprop="url">
					<span class="ap5-view-pack-name">{$productsPack.packObj->name|escape:'html':'UTF-8'}</span>
				</a>
				{if $enableBuyThisPackButton && AdvancedPack::isValidPack($idPack, true) && AdvancedPack::isInStock($idPack)}
