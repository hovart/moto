											...
											{foreach $languages as $language}
												<div class="lang_{$language.id_lang}" style="display:{if $language.id_lang == $defaultFormLanguage}block{else}none{/if}; float: left;">
													{if $input.type == 'tags'}
														...
														{/literal}
													{/if}
                                                    {assign var='valu' value=$input.name }
                                                    {assign var='valu' value=$valu|cat:'_'}
                                                    {assign var='valu' value=$valu|cat:$language.id_lang}
													{assign var='value_text' value=$fields_value[$valu]}
										...
										<div class="translatable">
											{foreach $languages as $language}
                                                {if isset($input.display_image) && $input.display_image}
                                                    {assign var='test' value=$input.name }
                                                    {assign var='test' value=$test|cat:'_'}
                                                    {assign var='test' value=$test|cat:$language.id_lang}

                                                {/if}
												<div class="lang_{$language.id_lang}" id="{$input.name}_{$language.id_lang}" style="display:{if $language.id_lang == $defaultFormLanguage}block{else}none{/if}; float: left;">
                                                    {if isset($fields_value[$test].image) && $fields_value[$test].image}
                                                        <div  id="image">
                                                            {$fields_value[$test].image}
                                                        </div><br />
                                                    {/if}
