...
class MGDBOTTOMSHOP extends Module
{
	public function __construct()
	{
		$this->name = 'mgdbottomshop';
		$this->description = 'Affichage des coordonnes magasins';
		$this->tab = 'front_office_features';
		$this->version = '1.1.1';
		$this->author = 'emotionLoop';
		$this->need_instance = 0;
		$this->ps_versions_compliancy = array('min' => '1.5', 'max' => '1.6');
		$this->monolanguage_content = 'Mgdbottomshop_MONOLANGUAGE';
		$this->text_editor_content = 'Mgdbottomshop_TEXTEDITOR';
		$this->content_wrapper = 'Mgdbottomshop_CONTENTWRAPPER';
		$this->content_wrapper_class = 'Mgdbottomshop_CONTENTWRAPPER_CLASS';
		$this->content_wrapper_id = 'Mgdbottomshop_CONTENTWRAPPER_ID';
		$this->bootstrap = true;
		$this->_html = '';
		...
		$this->displayName = $this->l('mgdBottomShop');
		$this->description = $this->l('Affichage des coordonnes magasins');
	...
	public function install()
	{
		...
		if (!parent::install() || !$this->registerHook(array('header', 'TopFooter')) || !$this->registerHook(array('footer', 'TopFooter')) || !MGDBOTTOMSHOPModel::createTables())
			return false;
	...
	public function addFilesToTemplate($path = null)
	{
		...
		$files_data = MGDBOTTOMSHOPModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id );
	...
	{
		$content_query = MGDBOTTOMSHOPModel::getContent( $this->selected_store_id, $this->selected_language_id );
		$pre_content = '';
		$pos_content = '';
		...
		if (Tools::isSubmit('submit'.$this->name))
		{
				...
				&& (int)Configuration::get('PS_LANG_DEFAULT') == $this->selected_language_id ))
			{
				...
				if (Tools::getValue('content_text') !== false && $this->ignore_changes_content_changes == false)
					MGDBOTTOMSHOPModel::setContent( Tools::getValue('content_text'), $this->selected_store_id, $this->selected_language_id );

				//store the files to be used
				if (Tools::getValue('headerFiles') !== false)
					MGDBOTTOMSHOPModel::setFiles(
	...
	public function displayForm()
	{
		...
		$helper->fields_value['headerFiles[]'] = $this->processSelectFilesForMultiselect(
																MGDBOTTOMSHOPModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id )
																);
...
class MGDBOTTOMSHOPModel extends ObjectModel
{
	...
	public static function createTables()
	{
		...
		return ( MGDBOTTOMSHOPModel::createFilesTable()
				&& MGDBOTTOMSHOPModel::createContentTable());
	}
