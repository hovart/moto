...
{if $nbr_filterBlocks != 0}
        ...
        <div id="layered_block_left" class="block">
            ...
            <div class="block_content">
                ...
                {if $isMobile == 'mobile'}
                        <div id="filtersGroup">
                            <div class="filter-toggle" role="tab">
                                <a role="button" data-toggle="collapse" data-parent="#filtersGroup" href="#filterMobile" aria-expanded="true" aria-controls="collapseOne">
                                    {if $lang_iso == de }Suche filtern{else}Filtrer la recherche {/if}<i class="fa fa-chevron-down "></i>
                                </a>
                            </div>
                            <div id="filterMobile" class="panel-collapse collapse" role="tabpanel">
                    {else}{/if}
                    <div>
                                                    ...
                                                    {foreach from=$filter.values key=id_value item=value}
                                                        {if $id_value == $filter_key && !is_numeric($filter_value) && ($filter.type eq 'id_attribute_group' || $filter.type eq 'id_feature') || $id_value == $filter_value && $filter.type neq 'id_attribute_group' && $filter.type neq 'id_feature'}
                                                            <li>
                                                                ...
                                                                <a href="#" data-rel="layered_{$filter.type_lite}_{$id_value}" title="{l s='Cancel' mod='blocklayered'}"><i class="fa fa-times"></i></a>
                                                            </li>
                                                        {/if}
                                                    {/foreach}
                                                {/if}
                        ...
                        {foreach from=$filters item=filter}
                            {if isset($filter.values)}
                                ...
                                <div class="layered_subtitle_heading">
                                    <a class="layered_subtitle" role="button" data-toggle="collapse" href="#collapseMgd_{$filter.type}_{$filter.id_key}">{$filter.name|escape:'html':'UTF-8'}</a>
                                    {* <span class="layered_close"><a href="#" data-rel="ul_layered_{$filter.type}_{$filter.id_key}">X</a></span> *}
                                </div>
                                <div class="collapse {if $isMobile != 'mobile'}in{/if}" id="collapseMgd_{$filter.type}_{$filter.id_key}">
