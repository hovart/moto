...
var tipsIsShown = false;
var tipsIsShownClick = false;
function displayTips(e,message,eventShow,eventHide,showTipsOnClick) {
	$jqPm(e).bind(eventShow, function() {
		if(tipsIsShownClick) { tipsIsShown = true; return; }
		if(tipsIsShown) return;
		tipsIsShown = true;
		parent.display(message);
	});
	$jqPm(e).bind(eventHide, function() {
		parent.hide();
		tipsIsShown = false;
		tipsIsShownClick = false;
	});
	if(showTipsOnClick) {
		$jqPm(e+'-tips').bind('click', function() {
			if(tipsIsShown || tipsIsShownClick) {
				parent.hide();
				var returnAfter = true;
				if(tipsIsShownClick && e != tipsIsShownClick) returnAfter = false;
				tipsIsShownClick = false;
				tipsIsShown = false;
				if(returnAfter)
					return;
