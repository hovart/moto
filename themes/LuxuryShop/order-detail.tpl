...
<div class="box box-small clearfix">
	...
	<form id="submitReorder" action="{if isset($opc) && $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}" method="post" class="submit">
		...
		<a href="#" onclick="$(this).closest('form').submit(); return false;" class="button btn btn-default button-medium pull-right">{l s='Reorder'}<i class="fa fa-chevron-right right"></i></a>
	</form>	
	{/if}
	<p class="dark">
...
<div class="info-order box">
	...
	{/if}
	{if $order->recyclable}
	<p><i class="fa fa-repeat left"></i>&nbsp;{l s='You have given permission to receive your order in recycled packaging.'}</p>
	{/if}
	{if $order->gift}
		<p><i class="fa fa-gift left"></i>&nbsp;{l s='You have requested gift wrapping for this order.'}</p>
		<p><strong class="dark">{l s='Message'}</strong> {$order->gift_message|nl2br}</p>
	{/if}
</div>

{if count($order_history)}
<h3 class="subheading">{l s='Follow your order\'s status step-by-step'}</h3>
	...
	<table class="detail_step_by_step table table-bordered">
		...
		{foreach from=$order_history item=state name="orderStates"}
			<tr class="{if $smarty.foreach.orderStates.first}first_item{elseif $smarty.foreach.orderStates.last}last_item{/if} {if $smarty.foreach.orderStates.index % 2}alternate_item{else}item{/if}">
				...
				<td><span{if isset($state.color) && $state.color} style="background-color:{$state.color|escape:'html':'UTF-8'}; color:#fff; padding:5px;"{/if} class="label{if isset($state.color) && Tools::getBrightness($state.color) > 128} dark{/if}">{$state.ostate_name|escape:'html':'UTF-8'}</span></td>
			</tr>
		{/foreach}
		</tbody>
	</table>
...
<div class="adresses_bloc">
	<div class="row">
		<div class="col-xs-12 col-md-6"{if $order->isVirtual()} style="display:none;"{/if}>
			<ul class="address alternate_item box">
				<li class="address_title">{l s='Delivery address'} ({$address_delivery->alias})</li>
		...
		<div class="col-xs-12 col-md-6">
			<ul class="address item {if $order->isVirtual()}full_width{/if} box">
				<li class="address_title">{l s='Invoice address'} ({$address_invoice->alias})</li>
...
<div id="order-detail-content" class="table_block table-responsive">
	<table class="table table-bordered">
		...
		<tfoot>
			...
			<tr class="item">
				<td colspan="{if $return_allowed}5{else}4{/if}">
					{l s='Items'} {if $use_tax}{l s='(tax incl.)'}{/if}
				</td>
				<td>
			...
			<tr class="item">
				<td colspan="{if $return_allowed}5{else}4{/if}">
					{l s='Shipping & handling'} {if $use_tax}{l s='(tax incl.)'}{/if}
				</td>
				<td>
...
{if $return_allowed}
	<div id="returnOrderMessage">
		<h3 class="subheading bottom-indent">{l s='Merchandise return'}</h3>
		...
		<p class="form-group">
			<button type="submit" name="submitReturnMerchandise" class="button">{l s='Make an RMA slip'}</button>
...
{if !$is_guest}
	{if count($messages)}
	<h3 class="subheading">{l s='Messages'}</h3>
	 <div class="table_block">
		<table class="detail_step_by_step table table-bordered">
			<thead>
				<tr>
	...
	<form action="{$link->getPageLink('order-detail', true)|escape:'html':'UTF-8'}" method="post" class="std" id="sendOrderMessage">
		<h3 class="subheading bottom-indent">{l s='Add a message'}</h3>
		...
		<div class="submit">
			...
			<button type="submit" name="submitMessage" class="button">{l s='Send'}</button>
		</div>
	</form>
{else}
<p class="alert alert-info"><i class="fa fa-info left"></i>{l s='You cannot return merchandise with a guest account'}</p>
{/if}
{/if}