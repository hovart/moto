...
class totsplashscreenModule extends Module {
	...
	protected function sendVoucher($email) {
		...
		if ($discount = Configuration::get('NW_VOUCHER_CODE'))
			return Mail::Send((int) $cookie->id_lang, 'newsletter_voucher', Mail::l('Newsletter voucher', (int) $cookie->id_lang), array('{discount}' => $discount), $email, NULL, NULL, NULL, NULL, NULL, '/home/ddauteuil/www/mgd-beta/modules/totsplashscreen'.'/mails/');
		return false;
	}
	...
	public function getContent() {
		...
		if (version_compare(_PS_VERSION_, '1.4.9', '<')) {
			$smarty->template_dir = '/home/ddauteuil/www/mgd-beta/modules/totsplashscreen'.'/views/templates/back';
		}
		...
		if (Tools::isSubmit('TOTsubmitNewsletter')) {
			...
			elseif (isset($this->valid)) {
				...
				if (Configuration::get('NW_CONFIRMATION_EMAIL') AND $this->getValue('TOTaction') AND (int) ($this->getValue('TOTaction') == 0))
					Mail::Send((int) ($params['cookie']->id_lang), 'newsletter_conf', Mail::l('Newsletter confirmation'), array(), pSQL($this->getValue('TOTemail')), NULL, NULL, NULL, NULL, NULL, '/home/ddauteuil/www/mgd-beta/modules/totsplashscreen'.'/mails/');
	...
	protected function uploadImage($file) {
		if (!empty($file)) {
			...
			// Path to img
			$path = '/home/ddauteuil/www/mgd-beta/modules/totsplashscreen'.'/upload/'.$name;
