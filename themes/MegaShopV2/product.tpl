...
/***** OVERRIDE BY DETAILS *****/
var availableNowValue = {if $product->available_now != ''}'{$product->available_now}'{else}'{l s='Available'}'{/if};
var availableLaterValue = {if $product->available_later != ''}'{$product->available_later}'{else}'{l s='Available within 10 days'}'{/if};
/***** OVERRIDE BY DETAILS *****/
...
<div id="primary_block" class="clearfix" itemscope="" itemtype="http://schema.org/Product">
	...
	<div id="pb-right-column">
				...
				{if isset($images)}
					...
					<li id="thumbnail_{$image.id_image}">
						<a href="{$link->getImageLink($product->link_rewrite, $imageIds, 'thickbox_default')}" rel="other-views" class="thickbox{if $smarty.foreach.thumbnails.first} shown{/if}" title="{$image.legend|htmlspecialchars}">
							<img id="thumb_{$image.id_image}" src="{$link->getImageLink($product->link_rewrite, $imageIds, 'thickbox_default')}" alt="{$image.legend|htmlspecialchars}" height="auto" width="100%" />
						</a>
					</li>
					{/foreach}
				{/if}
		...
		<!-- Lien vers guide des tailles -->
		<div id="guide-tailles">
	...
	<div id="pb-left-column">
		...
		<form id="buy_block" {if $PS_CATALOG_MODE AND !isset($groups) AND $product->quantity > 0}class="hidden"{/if} action="{$link->getPageLink('cart')|escape:'html'}" method="post">
			...
			<p id="availability_statut">
				...
				<span id="availability_value"{if $product->quantity <= 0} class="warning_inline"{/if}>{if $product->quantity <= 0}{if $product->available_later != ''}{$product->available_later}{else}{l s='Available within 10 days'}{/if}{else}{if $product->available_now != ''}{$product->available_now}{else}{l s='Available'}{/if}{/if}</span>
			</p>
		...
		<div class="content_prices clearfix">
			...
			<div class="price">
				...
				{if $priceDisplay >= 0 && $priceDisplay <= 2}
					{if $productPriceWithoutReduction > $productPrice}<span id="old_price_display">{convertPrice price=$productPriceWithoutReduction}</span>{/if}
					<!-- {if $tax_enabled && $display_tax_label == 1}{if $priceDisplay == 1}{l s='tax excl.'}{else}{l s='tax incl.'}{/if}{/if} -->
				{/if}
				</p>
				
				{if $priceDisplay == 2}
					<span id="pretaxe_price" itemprop="price"><span id="pretaxe_price_display">{convertPrice price=$product->getPrice(false, $smarty.const.NULL)}</span>&nbsp;{l s='tax excl.'}</span>
				{/if}
			...
			<p id="quantity_wanted_p"{if (!$allow_oosp && $product->quantity <= 0) OR $virtual OR !$product->available_for_order OR $PS_CATALOG_MODE} style="display: none;"{/if}>
				...
				<input class="qty-decrease" value="-" type="button" onclick="qtyDown()" />
				<input type="text" name="qty" id="quantity_wanted" class="text" value="{if isset($quantityBackup)}{$quantityBackup|intval}{else}{if $product->minimal_quantity > 1}{$product->minimal_quantity}{else}1{/if}{/if}" size="2" maxlength="3" {if $product->minimal_quantity > 1}onkeyup="checkMinimalQuantity({$product->minimal_quantity});"{/if} />
				<input class="qty-increase" value="+"type="button" onclick="qtyUp()" />
			</p>
	...
	<ul id="more_info_tabs" class="idTabs idTabsShort clearfix">
		...
		{if $features}<li><a id="more_info_tab_data_sheet" href="#idTab1">{l s='Data sheet'}</a></li>{/if}
	...
	{if isset($features) && $features}
		...
		<div id="idTab2">
			...
			{foreach from=$features item=feature name=featureLoop}
				...
				<tr class="{if $smarty.foreach.featureLoop.index % 2}alternate_item{/if}"> 
					<td width="50%" class="feat_name">{$feature.name|escape:'htmlall':'UTF-8'}</td>
					<td width="50%" class="feat_value">{$feature.value|escape:'htmlall':'UTF-8'}</td>
				</tr>
				{/if}
			{/foreach}
			</table>
						...
						{if ($accessory.allow_oosp || $accessory.quantity_all_versions > 0 || $accessory.quantity > 0) AND $accessory.available_for_order AND !isset($restricted_country_mode)}
							...
							<li class="ajax_block_product product_accessories_description clearfix">
								...
								<p class="s_title_block">
									<a class="prod_name" href="{$accessoryLink|escape:'htmlall':'UTF-8'}">{$accessory.name|escape:'htmlall':'UTF-8'|truncate:28:'...'}</a>
