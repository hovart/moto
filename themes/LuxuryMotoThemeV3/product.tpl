...
<div itemscope itemtype="http://schema.org/Product">
	...
	<div class="primary_block">
		...
		<div class="pb-left-column">
			<!-- product img-->        
			<div id="image-block">
				{if $product->new}<span class="new-box">{l s='New'}</span>{/if}
				{if $product->on_sale || ($product->specificPrice && $product->specificPrice.reduction && $productPriceWithoutReduction > $productPrice)}<span class="sale-box">
						{strip}
				...
				<div id="views_block" class="clearfix {if isset($images) && count($images) < 2}hidden{/if}">
					{if isset($images) && count($images) > 4}
						<span class="view_scroll_spacer">
							<a id="view_scroll_left" class="" title="{l s='Other views'}" href="javascript:{ldelim}{rdelim}"></a>
						</span>
					{/if}
					<div id="thumbs_list">
					...
					{if isset($images) && count($images) > 4}
						<a id="view_scroll_right" title="{l s='Other views'}" href="javascript:{ldelim}{rdelim}"></a>
					{/if}
				</div> <!-- end views-block -->
				<!-- end thumbnails -->
			{/if}
		...
		<div class="pb-right-column">
			<div class="mgd-product-essential">
				<h1 class="page-heading" itemprop="name">{$product->name|escape:'html':'UTF-8'}</h1>
					{if $product->manufacturer_name && !$content_only}
					<a href="{$link->getmanufacturerLink($product->id_manufacturer, $product->link_rewrite)|escape:'html':'UTF-8'}"  class="manufacturerProduct" title="{$product->manufacturer_name|escape:'html':'UTF-8'}">
						<img src="{$base_dir_ssl}img/m/{$product_manufacturer->id}.jpg" alt="{$product->manufacturer_name|escape:'html':'UTF-8'}"/>
					</a>
					{/if}
				...
				<div class="mgd-details-product">
					...
					<p id="product_reference" class="product_p">
						<label class="attribute_label">{l s='Reference:'}</label>
						<span class="editable" itemprop="sku"{if !empty($product->reference) && $product->reference} content="{$product->reference}{/if}">{if !isset($groups)}{$product->reference|escape:'html':'UTF-8'}{/if}</span>
					</p>
					{/if}
					{if !$product->is_virtual && $product->condition}
					<p id="product_condition">
						<label class="attribute_label">{l s='Condition'} :</label>
					...
					<form id="buy_block"{if $PS_CATALOG_MODE && !isset($groups) && $product->quantity > 0} class="hidden"{/if} action="{$link->getPageLink('cart')|escape:'html':'UTF-8'}" method="post">
						...
						<div class="product_attributes">
							...
							<!-- quantity wanted -->
							{if !$PS_CATALOG_MODE}
							<p id="quantity_wanted_p" class="product_p"{if (!$allow_oosp && $product->quantity <= 0) || !$product->available_for_order || $PS_CATALOG_MODE} style="display: none;"{/if}>
								<label for="quantity_wanted" class="attribute_label mtop">{l s='Quantity'} :</label>
									...
									{foreach from=$groups key=id_attribute_group item=group}
										{if $group.attributes|@count}
											<fieldset class="attribute_fieldset">
												<label class="attribute_label mtop" {if $group.group_type != 'color' && $group.group_type != 'radio'}for="group_{$id_attribute_group|intval}"{/if}>{$group.name|escape:'html':'UTF-8'}&nbsp;:</label>
			...
			<!--Guides des tailles-->
			<!--casques-->
			{if $category->id == 191 || $category->id == 203 || $category->id == 192 || $category->id == 197 || $category->id == 198 || $category->id == 200 || $category->id == 201 || $category->id == 244 || $category->id == 282 || $category->id == 202 || $category->id == 294}
			...
			<!--gants-->
			{elseif $category->id == 216 || $category->id == 226 || $category->id == 218 || $category->id == 217 || $category->id == 223 || $category->id == 224 || $category->id == 296 || $category->id == 225 }
			<div class="guide-buttons">
				{if $lang_iso == de }
			...
			<!--vestes-->
			{elseif $category->id == 194 || $category->id == 195 || $category->id == 196 }
				<div class="guide-buttons">
					{if $lang_iso == de }
				...
				{if isset($attachments) && $attachments}
					<ul id="idTab9">
						...
						<li>
							...
							<a class="button" href="{$link->getPageLink('attachment', true, NULL, "id_attachment={$attachment.id_attachment}")|escape:'html':'UTF-8'}">
							...
							{if ($accessory.allow_oosp || $accessory.quantity_all_versions > 0 || $accessory.quantity > 0) && $accessory.available_for_order && !isset($restricted_country_mode)}
								...
								<div class="item-content">
									<a href="{$accessoryLink|escape:'html':'UTF-8'}" title="{$accessory.legend|escape:'html':'UTF-8'}" class="product_image">
										<img src="{$link->getImageLink($accessory.link_rewrite, $accessory.id_image, 'home_default')|escape:'html'}" alt="{$accessory.legend|escape:'htmlall':'UTF-8'}" width="{$homeSize.width}" height="{$homeSize.height}" />
									</a>
									<p class="product_name">
										<a class="product-name" href="{$accessoryLink|escape:'html':'UTF-8'}">{$accessory.name|escape:'html':'UTF-8'|truncate:30:'...'}</a>
									</p>
