 ...
 *         			 		  	    
 */
class AdminPostFinanceCwMotoController extends AdminController
{
	public function __construct() {
		$this->className = 'AdminPostFinanceCwMotoController';
		parent::__construct();
		$this->context->smarty->addTemplateDir($this->getTemplatePath());
		$this->tpl_folder = 'postfinancecw_moto/';
		$this->bootstrap = true;
	}
	...
	public function initContent()
	{
		...
		if (isset($_GET['cw_transaction_id'])) {
			...
			$dbTransaction = PostFinanceCw_Entity_Transaction::loadById($_GET['cw_transaction_id']);
			if ($dbTransaction->getTransactionObject()->isAuthorized()) {
				...
				$params['vieworder'] = '1';
				header('Location: ' . PostFinanceCw::getAdminUrl('AdminOrders', $params));
				die();
			}
			else {
		...
		if ($failedTransaction !== NULL) {
			$moduleInstance = PostFinanceCw_PaymentMethod::getInstanceById($dbTransaction->getModuleId());
		}
		else {
		...
		/* @var $moduleInstance PostFinanceCw_PaymentMethod */
		...
		$adapter = PostFinanceCw_Util::getAuthorizationAdapter(Customweb_Payment_Authorization_Moto_IAdapter::AUTHORIZATION_METHOD_NAME);
		...
		if ($isSupported) {
			...
			if ($visibleFormFields !== null && count($visibleFormFields) > 0) {
				$renderer = new PostFinanceCw_FormRenderer($moduleInstance->getPaymentMethodName());
