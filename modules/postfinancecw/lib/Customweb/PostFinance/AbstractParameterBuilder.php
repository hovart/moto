...
require_once 'Customweb/PostFinance/BackendOperation/Form/StaticTemplate.php';
...
abstract class Customweb_PostFinance_AbstractParameterBuilder {
	...
	public function __construct(Customweb_Payment_Authorization_ITransaction $transaction, Customweb_DependencyInjection_IContainer $container){
		...
		$this->configuration = new Customweb_PostFinance_Configuration($this->container->getBean('Customweb_Payment_IConfigurationAdapter'));
	}
	...
	protected function getPspParameter(){
		...
		$configuredCurrencies = $storage->read('PostFinance_PSPIDs', 'configuredCurrencies');
	...
	protected function getOrderIdParameter(){
		...
		if ($id === null) {
			$id = Customweb_PostFinance_Util::substrUtf8($this->getTransactionIdAppliedSchema(), 0, 30);
	...
	protected function getOrderDescriptionParameter(){
		$desc = Customweb_PostFinance_Util::substrUtf8($this->getOrderDescriptionAppliedSchema(), 0, 30);
		return array(
			'COM' => $desc 
		);
	}
	...
	protected function getTransactionIdAppliedSchema(){
		return Customweb_PostFinance_Util::applyOrderSchema($this->getConfiguration(), $this->getTransaction()->getExternalTransactionId());
	}

	protected function getOrderDescriptionAppliedSchema(){
		return Customweb_PostFinance_Util::applyOrderDescriptionSchema($this->getConfiguration(), 
				$this->getTransaction()->getExternalTransactionId());
	}

	/**
	 *
	 * @return Customweb_PostFinance_Method_Factory
	 */
	public function getPaymentMethodFactory(){
		return $this->getContainer()->getBean('Customweb_PostFinance_Method_Factory');
	}
	...
	protected function getAliasManagerParameters(){
		...
		if ($this->getTransactionContext()->getAlias() == 'new' || ($this->getTransactionContext()->createRecurringAlias() && $this->getTransactionContext()->getAlias() == null)) {
			$parameters = $this->getPaymentMethod()->getAliasCreationParameters($this->getTransaction());
				 ...
				 $this->getTransactionContext()->getAlias() instanceof Customweb_PostFinance_Authorization_Transaction) {
	...
	protected function getCapturingModeParameter(){
		...
		if (strtolower($capturingMode) == 'direct') {
			return array(
				'OPERATION' => Customweb_PostFinance_IAdapter::OPERATION_DIRECT_SALE 
			);
		}
		else {
			return array(
				'OPERATION' => Customweb_PostFinance_IAdapter::OPERATION_AUTHORISATION 
			);
		}
	}
	...
	protected function getCustomerParameters(){
		return array(
			'CN' => $this->cleanStringParameter(Customweb_PostFinance_Util::substrUtf8(
			...
			'EMAIL' => Customweb_PostFinance_Util::substrUtf8($this->getTransactionContext()->getOrderContext()->getCustomerEMailAddress(), 0, 
					50),
			'OWNERZIP' => Customweb_PostFinance_Util::substrUtf8($this->getTransactionContext()->getOrderContext()->getBillingPostCode(), 0, 
					10),
			'OWNERADDRESS' => $this->cleanStringParameter(Customweb_PostFinance_Util::substrUtf8($this->getTransactionContext()->getOrderContext()->getBillingStreet(), 0, 
					50)),
			'OWNERTOWN' => $this->cleanStringParameter(Customweb_PostFinance_Util::substrUtf8($this->getTransactionContext()->getOrderContext()->getBillingCity(), 0, 40)),
	...
	protected function getDeliveryAndInvoicingParameters(){
		$parameters = array(
			'ECOM_BILLTO_POSTAL_CITY' => $this->cleanStringParameter(Customweb_PostFinance_Util::substrUtf8(
			...
			'ECOM_BILLTO_POSTAL_NAME_FIRST' => $this->cleanStringParameter(Customweb_PostFinance_Util::substrUtf8(
					$this->getTransactionContext()->getOrderContext()->getBillingFirstName(), 0, 35)),
			'ECOM_BILLTO_POSTAL_NAME_LAST' => $this->cleanStringParameter(Customweb_PostFinance_Util::substrUtf8(
					$this->getTransactionContext()->getOrderContext()->getBillingLastName(), 0, 35)),
			'ECOM_BILLTO_POSTAL_POSTALCODE' => $this->cleanStringParameter(Customweb_PostFinance_Util::substrUtf8(
					$this->getTransactionContext()->getOrderContext()->getBillingPostCode(), 0, 10)),
			'ECOM_BILLTO_POSTAL_STREET_LINE1' => $this->cleanStringParameter(Customweb_PostFinance_Util::substrUtf8(
					$this->getTransactionContext()->getOrderContext()->getBillingStreet(), 0, 35)),
			
			'ECOM_SHIPTO_POSTAL_CITY' => $this->cleanStringParameter(Customweb_PostFinance_Util::substrUtf8(
			...
			'ECOM_SHIPTO_POSTAL_NAME_FIRST' => $this->cleanStringParameter(Customweb_PostFinance_Util::substrUtf8(
					$this->getTransactionContext()->getOrderContext()->getShippingFirstName(), 0, 35)),
			'ECOM_SHIPTO_POSTAL_NAME_LAST' => $this->cleanStringParameter(Customweb_PostFinance_Util::substrUtf8(
					$this->getTransactionContext()->getOrderContext()->getShippingLastName(), 0, 35)),
			'ECOM_SHIPTO_POSTAL_POSTALCODE' => $this->cleanStringParameter(Customweb_PostFinance_Util::substrUtf8(
					$this->getTransactionContext()->getOrderContext()->getShippingPostCode(), 0, 10)),
			'ECOM_SHIPTO_POSTAL_STREET_LINE1' => $this->cleanStringParameter(Customweb_PostFinance_Util::substrUtf8(
					$this->getTransactionContext()->getOrderContext()->getShippingStreet(), 0, 35)),
			'ECOM_SHIPTO_ONLINE_EMAIL' => Customweb_PostFinance_Util::substrUtf8(
	...
	protected final function getTemplateParameter(){		
		...
		if (!empty($templateUrl)) {
			...
			if($templateUrl === 'static') {
				$settingsHandler = $this->getContainer()->getBean('Customweb_Payment_SettingHandler');
				$fields = Customweb_PostFinance_BackendOperation_Form_StaticTemplate::getTemplateFields();
				$parameters = array();
