...
{
    // Process Module upgrade to 1.15.2

    require_once _PS_MODULE_DIR_.'advancedimporter/classes/cron.php';

    $cron = new Cron();
    $cron->description = $module->l('Clean old flows');
    ...
    $collection->where('callback', '=', 'Cleaner::exec');
    foreach ($collection as $cron) {
        $cron->callback = 'Cleaner::block';
        $cron->description = $module->l('Clean old blocks not attached to a flow');
        $cron->save();
    }
