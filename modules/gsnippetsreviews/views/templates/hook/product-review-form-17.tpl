...
{if !empty($bDisplayForm)}
	<div id="comment-form" class="block">
		<form method="post" id="bt_comment-form" name="bt_comment-form" onsubmit="oGsr.form('bt_comment-form', '{$sMODULE_URI|escape:'htmlall':'UTF-8'}', '', 'comment-form', 'comment-form', false, false, null, 'review', 'review', '#fancybox-content');return false;">
			...
			<h1 class="h1">{l s='Write your review' mod='gsnippetsreviews'}</h1>
			<div class="border-bottom clr_5"></div>
			<div class="product">
				<p class="description">
			...
			<div class="pull-left"><sup class="required">*</sup>&nbsp;<strong class="size12">{l s='Rate this product' mod='gsnippetsreviews'}:</strong></div>
			<div>{section loop=$iMaxRating name=note}<input class="star" type="radio" name="bt_rating" value="{$smarty.section.note.iteration|intval}" {if (!empty($iCustomerNote) && $iCustomerNote == $smarty.section.note.iteration) || (!empty($iPreSelectedRating) && $iPreSelectedRating == $smarty.section.note.iteration)}checked="checked"{/if}/>{/section}</div>

			{if !empty($iCustomerNote)}
				<div class="clr_10"></div>
				<div class="alert alert-info">{l s='You have already rated this product' mod='gsnippetsreviews'}{if !empty($bEnableComments)} {l s='but you can add a comment' mod='gsnippetsreviews'}{/if}</div>
			{/if}
				...
				<div id="footer">
					<p class="text-center">
						{if empty($iCustomerNote) || !empty($bEnableComments)}
							<button name="bt_comment-button" class="btn btn-primary" value="{l s='Send' mod='gsnippetsreviews'}"  onclick="oGsr.form('bt_comment-form', '{$sMODULE_URI|escape:'htmlall':'UTF-8'}', '', 'comment-form', 'comment-form', false, false, null, 'review', 'review', '#fancybox-content');return false;"><i class="material-icons">&#xE876;</i>{l s='Send' mod='gsnippetsreviews'}</button>
						{/if}
						&nbsp;
						<button class="btn btn-secondary" value="{l s='Cancel' mod='gsnippetsreviews'}"  onclick="$.fancybox.close();return false;" ><i class="material-icons">&#x2715;</i>{l s='Cancel' mod='gsnippetsreviews'}</button>
					</p>
					<div class="clr_20"></div>
				</div>
			</div>
