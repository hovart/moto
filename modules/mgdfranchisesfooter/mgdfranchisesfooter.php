...
class MGDFRANCHISESFOOTER extends Module
{
	public function __construct()
	{
		$this->name = 'mgdfranchisesfooter';
		$this->description = 'Affiches les liens vers franchises dans le footer';
		$this->tab = 'front_office_features';
		$this->version = '1.1.1';
		$this->author = 'emotionLoop';
		$this->need_instance = 0;
		$this->ps_versions_compliancy = array('min' => '1.5', 'max' => '1.6');
		$this->monolanguage_content = 'Mgdfranchisesfooter_MONOLANGUAGE';
		$this->text_editor_content = 'Mgdfranchisesfooter_TEXTEDITOR';
		$this->content_wrapper = 'Mgdfranchisesfooter_CONTENTWRAPPER';
		$this->content_wrapper_class = 'Mgdfranchisesfooter_CONTENTWRAPPER_CLASS';
		$this->content_wrapper_id = 'Mgdfranchisesfooter_CONTENTWRAPPER_ID';
		$this->bootstrap = true;
		$this->_html = '';
		...
		$this->displayName = $this->l('mgdFranchisesFooter');
		$this->description = $this->l('Affiches les liens vers franchises dans le footer');
	...
	public function install()
	{
		...
		if (!parent::install() || !$this->registerHook('header') || !$this->registerHook('footer') || !MGDFRANCHISESFOOTERModel::createTables())
			return false;
	...
	public function uninstall()
	{
		$this->_clearCache('template.tpl');
		if (!parent::uninstall()
			|| !MGDFRANCHISESFOOTERModel::DropTables())
		{
	...
	public function addFilesToTemplate($path = null)
	{
		...
		$files_data = MGDFRANCHISESFOOTERModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id );
	...
	{
		$content_query = MGDFRANCHISESFOOTERModel::getContent( $this->selected_store_id, $this->selected_language_id );
		$pre_content = '';
		$pos_content = '';
		...
		if (Tools::isSubmit('submit'.$this->name))
		{
				...
				&& (int)Configuration::get('PS_LANG_DEFAULT') == $this->selected_language_id ))
			{
				...
				if (Tools::getValue('content_text') !== false && $this->ignore_changes_content_changes == false)
					MGDFRANCHISESFOOTERModel::setContent( Tools::getValue('content_text'), $this->selected_store_id, $this->selected_language_id );

				//store the files to be used
				if (Tools::getValue('headerFiles') !== false)
					MGDFRANCHISESFOOTERModel::setFiles(
	...
	public function displayForm()
	{
		...
		$helper->fields_value['headerFiles[]'] = $this->processSelectFilesForMultiselect(
																MGDFRANCHISESFOOTERModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id )
																);
...
class MGDFRANCHISESFOOTERModel extends ObjectModel
{

	public static $definition = array(
		'table' => 'mgdfranchisesfooter',
		'primary' => 'file_id',
		'multishop' => true,
		'multilang' => true,
		'fields' => array(
	...
	public static function createTables()
	{
		...
		return ( MGDFRANCHISESFOOTERModel::createFilesTable()
				&& MGDFRANCHISESFOOTERModel::createContentTable());
	}
