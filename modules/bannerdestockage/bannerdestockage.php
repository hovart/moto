...
include_once(_PS_MODULE_DIR_.'bannerdestockage/bannerDestock.php');

class bannerdestockage extends Module
{
	private $_html = '';

	public function __construct()
	{
		$this->name = 'bannerdestockage';
		$this->tab = 'Blocks';
		$this->version = '1.0';
		$this->author = 'MGD';
		...
		$this->displayName = $this->l('bannerDestock homepage - MGD');
		$this->description = $this->l('Adds a block to display banners Destockage on homepage.');
	}
	...
	protected function createTables()
	{
		...
		$res = (bool)Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'bannerdestockage` (
				`id_bannerdestockage_slides` int(10) unsigned NOT NULL AUTO_INCREMENT,
				`id_shop` int(10) unsigned NOT NULL,
				PRIMARY KEY (`id_bannerdestockage_slides`, `id_shop`)
			) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
		');

		/* Slides configuration */
		$res &= Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'bannerdestockage_slides` (
			  `id_bannerdestockage_slides` int(10) unsigned NOT NULL AUTO_INCREMENT,
			  `position` int(10) unsigned NOT NULL DEFAULT \'0\',
			  `active` tinyint(1) unsigned NOT NULL DEFAULT \'0\',
			  PRIMARY KEY (`id_bannerdestockage_slides`)
			) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
		');

		/* Slides lang configuration */
		$res &= Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'bannerdestockage_slides_lang` (
			  `id_bannerdestockage_slides` int(10) unsigned NOT NULL,
			  `id_lang` int(10) unsigned NOT NULL,
			  `title` varchar(255) NOT NULL,
			  `url` varchar(255) NOT NULL,
			  `image` varchar(255) NOT NULL,
			  PRIMARY KEY (`id_bannerdestockage_slides`,`id_lang`)
			) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
		');
	...
	protected function deleteTables()
	{
		...
		{
			$to_del = new bannerDestock($slide['id_slide']);
			$to_del->delete();
		}
		return Db::getInstance()->execute('
			DROP TABLE IF EXISTS `'._DB_PREFIX_.'bannerdestockage`, `'._DB_PREFIX_.'bannerdestockage_slides`, `'._DB_PREFIX_.'bannerdestockage_slides_lang`;
		');
	}
	...
	private function _displayAddForm()
	{
		...
		if (Tools::isSubmit('id_slide') && $this->slideExists((int)Tools::getValue('id_slide')))
			$slide = new bannerDestock((int)Tools::getValue('id_slide'));
		...
		$this->_html .= '<p class="clear">'.$this->l('Recommended width: 340px.').'</p>';
	...
	private function _postProcess()
	{
		...
		{
			$slide = new bannerDestock((int)Tools::getValue('delete_id_slide'));
			$res = $slide->delete();
			$this->clearCache();
			if (!$res)
	...
	{
		if (!$this->isCached('bannerdestockage.tpl', $this->getCacheId()))
		{
			$slides = $this->getSlides(true);
			if (!$slides)
				return false;
			
			$this->smarty->assign('bannerdestockage_slides', $slides);
		}
	...
	public function hookDisplayHome()
	{
		...
		return $this->display(__FILE__, 'bannerdestockage.tpl', $this->getCacheId());
	}

	public function hookActionShopDataDuplication($params)
	{
		Db::getInstance()->execute('
		INSERT IGNORE INTO '._DB_PREFIX_.'bannerdestockage (id_bannerdestockage_slides, id_shop)
		SELECT id_bannerdestockage_slides, '.(int)$params['new_id_shop'].'
		FROM '._DB_PREFIX_.'bannerdestockage
		WHERE id_shop = '.(int)$params['old_id_shop']);
	}
	
	public function clearCache()
	{
		$this->_clearCache('bannerdestockage.tpl');
	}
	...
	public function getNextPosition()
	{
		$row = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow('
				...
				WHERE hss.`id_bannerdestockage_slides` = hs.`id_bannerdestockage_slides` AND hs.`id_shop` = '.(int)$this->context->shop->id
		);
	...
	public function getSlides($active = null)
	{
		...
		return Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT hs.`id_bannerdestockage_slides` as id_slide,
					   hssl.`image`,
					   hss.`position`,
					   hss.`active`,
					   hssl.`title`,
					   hssl.`url`
			FROM '._DB_PREFIX_.'bannerdestockage hs
			LEFT JOIN '._DB_PREFIX_.'bannerdestockage_slides hss ON (hs.id_bannerdestockage_slides = hss.id_bannerdestockage_slides)
			LEFT JOIN '._DB_PREFIX_.'bannerdestockage_slides_lang hssl ON (hss.id_bannerdestockage_slides = hssl.id_bannerdestockage_slides)
	...
	{
		$req = 'SELECT hs.`id_bannerdestockage_slides` as id_slide
				FROM `'._DB_PREFIX_.'bannerdestockage` hs
				WHERE hs.`id_bannerdestockage_slides` = '.(int)$id_slide;
