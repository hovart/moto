...
class Blocklateralreinsurance extends Module
{
	...
	public function getContent()
	{
			...
			if ($lateralreinsurance->validateFields(false) && $lateralreinsurance->validateFieldsLang(false))
			{
				...
				if (isset($_FILES['image']) && isset($_FILES['image']['tmp_name']) && !empty($_FILES['image']['tmp_name']))
				{
					...
					elseif (!($tmpName = tempnam(_PS_TMP_IMG_DIR_, 'PS')) || !move_uploaded_file($_FILES['image']['tmp_name'], $tmpName))
						return false;
					elseif (!ImageManager::resize($tmpName, dirname(__FILE__).'/img/lateralreinsurance-'.(int)$lateralreinsurance->id.'-'.(int)$lateralreinsurance->id_shop.'.jpg'))
						return false;
					unlink($tmpName);
		...
		{
			$lateralreinsurance = new lateralreinsuranceClass((int)$id_lateralreinsurance);
			if (file_exists(dirname(__FILE__).'/img/'.$lateralreinsurance->file_name))
				unlink(dirname(__FILE__).'/img/'.$lateralreinsurance->file_name);
