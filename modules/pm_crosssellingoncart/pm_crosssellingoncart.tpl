...
<div id="csoc-container" class="{if isset($on_product_page) && $on_product_page}page-product-box{/if} {$csoc_prefix|strtolower}">
			...
			{if !empty($csoc_display["{$csoc_prefix}_DISPLAY_BUTTON"])}
				{if ($cartProduct.quantity > 0 OR $cartProduct.allow_oosp) AND $cartProduct.customizable != 2}
					{if version_compare($smarty.const._PS_VERSION_, '1.5.0.0', '>=')}
						...
						<div class="addtocart_buttons_container"><a class="button ajax_add_to_cart_button exclusive" rel="ajax_id_product_{$cartProduct.id_product}" href="{$link->getPageLink('cart',false, NULL, "add&amp;id_product=$csoc_id_product&amp;token=$csoc_static_token", false)}" title="{l s='Add to cart' mod='pm_crosssellingoncart'}"><span></span>{l s='Add to cart' mod='pm_crosssellingoncart'}</a></div>
					{else}
						<div class="addtocart_buttons_container"><a class="exclusive ajax_add_to_cart_button" rel="ajax_id_product_{$cartProduct.id_product}" href="{$base_dir}cart.php?qty=1&amp;id_product={$cartProduct.id_product}&amp;add&amp;token={$csoc_static_token}" title="{l s='Add to cart' mod='pm_crosssellingoncart'}">{l s='Add to cart' mod='pm_crosssellingoncart'}</a></div>
					{/if}
				{else}
...
<script>
	...
	$csocjqPm(document).ready(function() {ldelim}
		$csocjqPm("#{$csoc_prefix}").owlCarousel({ldelim}
			...
			itemsTablet : [768,{$csoc_products_quantity_tablet}],
			itemsTabletSmall : false,
			itemsMobile : [479,{$csoc_products_quantity_mobile}],
			slideSpeed : 200,
			paginationSpeed : 800,
			autoPlay : true,
			stopOnHover : true,
