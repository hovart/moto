			...
			<h3>
				<span id="current_obj" style="font-weight: normal;"> 
					...
					<span class="breadcrumb item-2 ">
						{lcw s='View' mod='postfinancecw'}
						<img alt=">" style="margin-right:5px" src="../img/admin/separator_breadcrumb.png">
					</span>
					<span class="breadcrumb item-2 ">
						{lcw s='Capture' mod='postfinancecw'}
					</span>
				</span>
			</h3>
		</div>
...
<div class="buttons">
	<a href="{$link->getAdminLink('AdminPostFinanceCwTransaction')|escape:'htmlall':'UTF-8'}&transactionId={$transaction->getTransactionId()}&action=edit" class="button btn btn-default">{lcw s='Back' mod='postfinancecw'}</a>
</div>
<br />
...
{if $transaction->getTransactionObject()->isPartialCapturePossible()}
	<form method="POST" class="postfinancecw-line-item-grid" id="capture-form">
		<input type="hidden" id="postfinancecw-decimal-places" value="{Customweb_Util_Currency::getDecimalPlaces($transaction->getTransactionObject()->getCurrencyCode())}" />
		<input type="hidden" id="postfinancecw-currency-code" value="{strtoupper($transaction->getTransactionObject()->getCurrencyCode())}" />
		<table class="table table-striped table-condensed table-hover table-bordered">
			<thead>
				<tr>
					<th class="text-left">{lcw  mod='postfinancecw' s='Name'}</th>
					<th class="text-left">{lcw  mod='postfinancecw' s='SKU'}</th>
					<th class="text-left">{lcw  mod='postfinancecw' s='Type'}</th>
					<th class="text-left">{lcw  mod='postfinancecw' s='Tax Rate'}</th>
					<th class="text-right">{lcw  mod='postfinancecw' s='Quantity'}</th>
					<th class="text-right">{lcw  mod='postfinancecw' s='Total Amount (excl. Tax)'}</th>
					<th class="text-right">{lcw  mod='postfinancecw' s='Total Amount (incl. Tax)'}</th>
					</tr>
			</thead>
			...
			<tfoot>
				<tr>
					<td colspan="6" class="text-right">{lcw  mod='postfinancecw' s='Total Capture Amount'}:</td>
		...
		{if $transaction->getTransactionObject()->isCaptureClosable()}
			<div class="closable-box">
				<label for="close-transaction">{lcw  mod='postfinancecw' s='Close transaction for further captures'}</label>
				<input id="close-transaction" type="checkbox" name="close" value="on" />
			</div>
		{/if}
		
		<div class="text-right">
			<input type="submit" class="button btn btn-success" value="{lcw  mod='postfinancecw' s='Capture'}" />
		</div>
	</form>
{/if}
