...
class Customweb_PostFinance_Authorization_PaymentPage_Adapter extends Customweb_PostFinance_Authorization_AbstractAdapter 
	...
	public function getRedirectionUrl(Customweb_Payment_Authorization_ITransaction $transaction, array $formData) {
		if (!($transaction instanceof Customweb_PostFinance_Authorization_Transaction)) {
			throw new Customweb_Core_Exception_CastException('Customweb_PostFinance_Authorization_Transaction');
		}
		...
		try {
			...
			$builder = new Customweb_PostFinance_Authorization_PaymentPage_ParameterBuilder($transaction, $this->getContainer(), $formData);
