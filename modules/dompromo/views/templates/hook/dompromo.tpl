...
{assign var="heightli" value="`$heightslide -3`"} 
{math equation="haut - bord" haut=$heightslide bord=3 assign=heightli}

{if $vfproducts}
	<div id="special_block_right" class="block">
		<h4><a href="{$link->getModuleLink('dompromo', 'default', ['process' => 'flashsales'])}" title="{l s='Specials!' mod='dompromo'}">{l s='Specials!' mod='dompromo'}</a></h4>
		<div class="block_content">
			...
			{foreach from=$vfproducts item=product name=myLoop}
				{if $product.reduction_type == 'percentage'}
					...
					{else}
						<li class="dpclassli" style="width:{$widthslide}px;height:{$heightli}px;">
						<div class="dptitrepromo" style="color:#{$blocktexttimeColor};">
						{if $product.on_sale}{l s='On sale!' mod='dompromo'}{else}{l s='Specials' mod='dompromo'}{/if}</div>
							<div class="dpinfopromo" style="color:#{$blocktimeColor};" id="vftimera{$product.id}">{l s='Quickly !' mod='dompromo'}</div>
							<br />
					{/if}
					...
					<div><h5><div class="dptitreproduct" style="color:#{$blocktextColor};"><a class="dptitreproduct" href="{$product.link}" title="{$product.name|escape:htmlall:'UTF-8'}">{$product.name|truncate:25|escape:'htmlall':'UTF-8'}</a></div></h5>
					...
					{if $product.reduction_percent|intval}<div class="reduction dpreduction">(-{$product.reduction_percent|intval}%)</div>{/if}
					<div><div class="dpprixpromo" style="color:#{$blockpriceColor};">{if !$priceDisplay}{displayWtPrice p=$product.price}{else}{displayWtPrice p=$product.myprice}{/if}</div></div></div>
					</li>
				{/if}
			{/foreach}
			</ul>
		  ...
		  {if $csproducts}
		  	<p class="dpbtnavenir"><a href="{$link->getModuleLink('dompromo', 'default', ['process' => 'commingsoon'])}" title="{l s='Comming soon Specials' mod='dompromo'}"  class="button_large">{l s='Comming soon Specials' mod='dompromo'}</a></p>
		  {/if}
