...
<div id="featured-products_block_center" class="block products_block clearfix">
	<div id="titlefeatured-products">
		<h4 class="title_block"><span>{l s='Featured products' mod='homefeatured'}</span></h4>
	</div>
		...
		<div class="block_content">
			...
			{foreach from=$products item=product name=homeFeaturedProducts}
				...
				<li class="ajax_block_product {if $smarty.foreach.homeFeaturedProducts.first}first_item{elseif $smarty.foreach.homeFeaturedProducts.last}last_item{else}item{/if} {if $smarty.foreach.homeFeaturedProducts.iteration%$nbItemsPerLine == 0}last_item_of_line{elseif $smarty.foreach.homeFeaturedProducts.iteration%$nbItemsPerLine == 1} {/if} {if $smarty.foreach.homeFeaturedProducts.iteration > ($smarty.foreach.homeFeaturedProducts.total - $totModulo)}last_line{/if}">
					<a href="{$product.link|escape:'html'}" title="{$product.name|escape:html:'UTF-8'}" class="product_image"><img src="{$link->getImageLink($product.link_rewrite, $product.id_image, 'vignetteBig')|escape:'html'}" height="auto" width="100%" alt="{$product.name|escape:html:'UTF-8'}" />{if isset($product.new) && $product.new == 1}<span class="new">{l s='New' mod='homefeatured'}</span>{/if}</a>
					

					<div id="featured-products-description" style="">
						...
						<div class="product_desc" ><a href="{$product.link|escape:'html'}" title="{l s='More' mod='homefeatured'}">{$product.description_short|strip_tags|truncate:100:'...'}</a></div>
						<div class="homefeatured-btn">
