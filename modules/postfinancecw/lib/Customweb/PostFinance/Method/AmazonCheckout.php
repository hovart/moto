...
require_once 'Customweb/PostFinance/Method/LineItemBuilder/AmazonCheckout.php';
 ...
 */
class Customweb_PostFinance_Method_AmazonCheckout extends Customweb_PostFinance_Method_DefaultMethod {
	 ...
	 * (non-PHPdoc)
	 *
	 * @see Customweb_PostFinance_Method_DefaultMethod::processAuthorization()
	 */
	public function processAuthorization(Customweb_PostFinance_AbstractAdapter $adapter, Customweb_PostFinance_Authorization_Transaction $transaction, $parameters){
		...
		//handle step two
		$builder = new Customweb_PostFinance_Authorization_PaymentPage_ParameterBuilder($transaction, $this->getContainer(), array());
		$params = $builder->buildParameters();
		
		$url = $this->getGlobalConfiguration()->getBaseEndPointUrl() . Customweb_PostFinance_AbstractAdapter::URL_PAYMENT_PAGE;
	 ...
	 * (non-PHPdoc)
	 * 
	 * @see Customweb_PostFinance_Method_DefaultMethod::getAuthorizationParameters()
	 */
	...
	private function addSecondStepParameters(Customweb_PostFinance_Authorization_Transaction $transaction){
		//Line items
		$builder = new Customweb_PostFinance_Method_LineItemBuilder_AmazonCheckout($transaction->getTransactionContext()->getOrderContext());
	...
	private function isSecondStep(Customweb_PostFinance_Authorization_Transaction $transaction){
