   ...
   function _posts(){
         ...
         $array_error = array();
               ...
               if(!filter_var($mail,FILTER_VALIDATE_EMAIL)){
                     $array_error['mail'] = $SmartBlog->l('E-mail is invalid.');
                }
                if(Configuration::get('smartcaptchaoption') == '1'){
                    if($captcha != $m_captcha){
                   $array_error['captcha'] =  $SmartBlog->l('Captcha is not matching.');
                    }
                }
                

                if(is_array($array_error)&& count($array_error)) { 
                        $array_error['common'] = $SmartBlog->l('Warning: Please check required form below!');
                ...
                else
                {
                    ...
                    if(Configuration::get('smartacceptcomment') != '' && Configuration::get('smartacceptcomment') != null){
                       $value = Configuration::get('smartacceptcomment');
                        ...
                        if($bc->add()){
                        $array_success['common'] = $SmartBlog->l('Your comment is successfully submitted.');
                        $array_success['success'] =$SmartBlog->l('Your comment is successfully submitted.'); 
