...
{else}
	...
	{if !isset($empty) && !$PS_CATALOG_MODE}
		<span class="heading-counter" style="margin:0 0 15px; display:block;">{l s='Your shopping cart contains:'}
			...
			<form action="{if $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}" method="post" id="voucher">
				<fieldset>
					...
					<div class="voucher_box">
						...
						<button type="submit" name="submitAddDiscount" class="button">{l s='OK'}</button>
					</div>
				</fieldset>
			</form>
			{if $displayVouchers}
	...
	{if !$advanced_payment_api && ((!empty($delivery_option) && !isset($virtualCart)) OR $delivery->id || $invoice->id) && !$opc}
		<div class="order_delivery clearfix row">
			{if !isset($formattedAddresses) || (count($formattedAddresses.invoice) == 0 && count($formattedAddresses.delivery) == 0) || (count($formattedAddresses.invoice.formated) == 0 && count($formattedAddresses.delivery.formated) == 0)}
				{if $delivery->id}
					<div class="col-xs-12 col-md-6"{if !$have_non_virtual_products} style="display: none;"{/if}>
			...
			{else}
				{foreach from=$formattedAddresses key=k item=address}
					<div class="col-xs-12 col-md-6"{if $k == 'delivery' && !$have_non_virtual_products} style="display: none;"{/if}>
