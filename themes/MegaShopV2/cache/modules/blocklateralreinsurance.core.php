...
class BlocklateralreinsuranceModule extends Module
{
			...
			for ($i = 1; $i <= (int)$_POST['nbblocks']; $i++)
			{
				...
				if (isset($_FILES['info'.$i.'_file']) && isset($_FILES['info'.$i.'_file']['tmp_name']) && !empty($_FILES['info'.$i.'_file']['tmp_name']))
				{
					...
					elseif (!($tmpName = tempnam(_PS_TMP_IMG_DIR_, 'PS')) || !move_uploaded_file($_FILES['info'.$i.'_file']['tmp_name'], $tmpName))
						return false;
					elseif (!ImageManager::resize($tmpName, '/home/ddauteuil/www/mgd-beta/modules/blocklateralreinsurance'.'/img/'.$filename[0].'.jpg'))
						return false;
					unlink($tmpName);
				}
	...
	{
		$dir = opendir('/home/ddauteuil/www/mgd-beta/modules/blocklateralreinsurance'.'/img');
		while (false !== ($file = readdir($dir)))
		{
			$path = '/home/ddauteuil/www/mgd-beta/modules/blocklateralreinsurance'.'/img/'.$file;
	...
	public function getContent()
	{
			...
			if ($lateralreinsurance->validateFields(false) && $lateralreinsurance->validateFieldsLang(false))
			{
				...
				if (isset($_FILES['image']) && isset($_FILES['image']['tmp_name']) && !empty($_FILES['image']['tmp_name']))
				{
					...
					elseif (!($tmpName = tempnam(_PS_TMP_IMG_DIR_, 'PS')) || !move_uploaded_file($_FILES['image']['tmp_name'], $tmpName))
						return false;
					elseif (!ImageManager::resize($tmpName, '/home/ddauteuil/www/mgd-beta/modules/blocklateralreinsurance'.'/img/lateralreinsurance-'.(int)$lateralreinsurance->id.'-'.(int)$lateralreinsurance->id_shop.'.jpg'))
						return false;
					unlink($tmpName);
		...
		{
			$lateralreinsurance = new lateralreinsuranceClass((int)$id_lateralreinsurance);
			if (file_exists('/home/ddauteuil/www/mgd-beta/modules/blocklateralreinsurance'.'/img/'.$lateralreinsurance->file_name))
				unlink('/home/ddauteuil/www/mgd-beta/modules/blocklateralreinsurance'.'/img/'.$lateralreinsurance->file_name);
