...
/**
 * 
 * @Entity(tableName = 'postfinancecw_transactions')
 ...
 *
 */
class PostFinanceCw_Entity_Transaction extends Customweb_Payment_Entity_AbstractTransaction
{
	...
	protected function authorize(Customweb_Database_Entity_IManager $entityManager) {
		...
		if (empty($orderId)) {
			...
			$GLOBALS['postfinancecw_successful_transaction_object'] = $transactionObject;
			$employeeId = null;
			...
			$message = PostFinanceCw_Util::getOrderCreationMessage($employeeId);
			$paymentMethod->validateOrder(
					...
					$message,
					PostFinanceCw_Util::extractMailVariables($transactionObject),
			...
			if (count($messages) > 0 && method_exists('Mail', 'sendMailMessageWithoutHook')) {
				foreach ($messages as $message) {
					PostFinanceCw_Util::attachTransactionToMailMessage($this->getTransactionObject(), $message);
	...
	public static function getGridQuery() {
		return 'SELECT * FROM ' . _DB_PREFIX_ . 'postfinancecw_transactions WHERE ${WHERE} ${ORDER_BY} ${LIMIT}';
	}
	...
	public static function loadById($id, $cache = true) {
		return PostFinanceCw_Util::getEntityManager()->fetch('PostFinanceCw_Entity_Transaction', $id, $cache);
	}
	...
	public static function getTransactionsByOrderId($orderId, $loadFromCache = true) {
		return PostFinanceCw_Util::getEntityManager()->searchByFilterName('PostFinanceCw_Entity_Transaction', 'loadByOrderId', array('>orderId' => $orderId), $loadFromCache);
	}
	...
	public static function getTransactionsByCartId($cartId, $loadFromCache = true) {
		return PostFinanceCw_Util::getEntityManager()->searchByFilterName('PostFinanceCw_Entity_Transaction', 'loadByCartId', array('>cartId' => $cartId), $loadFromCache);
	}
	...
	public static function getTransactionsByOriginalCartId($cartId, $loadFromCache = true) {
		return PostFinanceCw_Util::getEntityManager()->searchByFilterName('PostFinanceCw_Entity_Transaction', 'loadByOriginalCartId', array('>cartId' => $cartId), $loadFromCache);
	}
	...
	public static function getTransactionsByCartOrOrder($cartId, $orderId, $loadFromCache = true) {
		return PostFinanceCw_Util::getEntityManager()->searchByFilterName('PostFinanceCw_Entity_Transaction', 'loadByCartOrOrder', array('>cartId' => $cartId, '>orderId' => $orderId), $loadFromCache);
	}
