...
include_once(_PS_MODULE_DIR_.'tptnsubbanner/TptnSubBan.php');

class TptnSubBanner extends Module
{
	private $_html = '';

	public function __construct()
	{
		$this->name = 'tptnsubbanner';
		$this->tab = 'Blocks';
		$this->version = '1.0';
		...
		$this->displayName = $this->l('Sub Banners');
		$this->description = $this->l('Adds a block to display banners below slider on homepage.');
	}
	...
	protected function createTables()
	{
		$res = (bool)Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'tptnsubbanner` (
		...
		$res &= Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'tptnsubbanner_slides` (
			  `id_homeslider_slides` int(10) unsigned NOT NULL AUTO_INCREMENT,
		...
		$res &= Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'tptnsubbanner_slides_lang` (
	...
	protected function deleteTables()
	{
		...
		{
			$to_del = new TptnSubBan($slide['id_slide']);
			$to_del->delete();
		}

		return Db::getInstance()->execute('
			DROP TABLE IF EXISTS `'._DB_PREFIX_.'tptnsubbanner`, `'._DB_PREFIX_.'tptnsubbanner_slides`, `'._DB_PREFIX_.'tptnsubbanner_slides_lang`;
		');
	}
	...
	public function getContent()
	{
		...
		elseif (Tools::isSubmit('addSlide') || (Tools::isSubmit('id_slide') && $this->slideExists((int)Tools::getValue('id_slide'))))
		{
			...
			{
				$associated_shop_ids = TptnSubBan::getAssociatedIdsShop((int)Tools::getValue('id_slide'));
	...
	private function _postProcess()
	{
		...
		{
			$slide = new TptnSubBan((int)Tools::getValue('id_slide'));
			if ($slide->active == 0)
				$slide->active = 1;
			else
				$slide->active = 0;
		...
		{
			$slide = new TptnSubBan((int)Tools::getValue('delete_id_slide'));
			$res = $slide->delete();
			$this->clearCache();
			if (!$res)
	...
	{
		if (!$this->isCached('tptnsubbanner.tpl', $this->getCacheId()))
		{
			...
			$this->smarty->assign(array('tptnsubbanner_slides' => $slides));
		}
	...
	public function hookDisplayHome()
	{
		...
		return $this->display(__FILE__, 'tptnsubbanner.tpl', $this->getCacheId());
	}

	public function clearCache()
	{
		$this->_clearCache('tptnsubbanner.tpl');
	}

	public function hookActionShopDataDuplication($params)
	{
		Db::getInstance()->execute('
			INSERT IGNORE INTO '._DB_PREFIX_.'tptnsubbanner (id_homeslider_slides, id_shop)
			SELECT id_homeslider_slides, '.(int)$params['new_id_shop'].'
			FROM '._DB_PREFIX_.'tptnsubbanner
	...
	public function getNextPosition()
	{
		$row = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow('
			...
			FROM `'._DB_PREFIX_.'tptnsubbanner_slides` hss, `'._DB_PREFIX_.'tptnsubbanner` hs
	...
	public function getSlides($active = null)
	{
		...
		return Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			hssl.`url`, hssl.`image`
			FROM '._DB_PREFIX_.'tptnsubbanner hs
			LEFT JOIN '._DB_PREFIX_.'tptnsubbanner_slides hss ON (hs.id_homeslider_slides = hss.id_homeslider_slides)
			LEFT JOIN '._DB_PREFIX_.'tptnsubbanner_slides_lang hssl ON (hss.id_homeslider_slides = hssl.id_homeslider_slides)
	...
	public function renderList()
	{
		...
		foreach ($slides as $key => $slide)
		{
			...
			$associated_shop_ids = TptnSubBan::getAssociatedIdsShop((int)$slide['id_slide']);
