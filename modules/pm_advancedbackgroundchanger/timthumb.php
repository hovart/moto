...
if(! isset($ALLOWED_SITES)) $ALLOWED_SITES = array ();
// -------------------------------------------------------------
...
class timthumb {
	...
	protected function processImageAndWriteToCache($localImage){
		...
		if (preg_match('#prestabox#', __FILE__)) {
			// Prestabox flock issue
	...
	protected function getLocalImagePath($src){
		...
		// Latest Alternative
		$base2 = realpath(dirname(__FILE__));
		foreach ($sub_directories as $sub){
			$base2 .= $sub . '/';
			$base2_and_src = realpath($base2 . $src);
			$this->debug(3, "Trying file as: " . $base2_and_src);
			if(file_exists($base2_and_src)){
				$this->debug(3, "Found file as: " . $base2_and_src);
				$real = $this->realpath($base2_and_src);
				if(stripos($real, $this->realpath(dirname(__FILE__))) === 0){
					return $real;
				} else {
					$this->debug(1, "Security block: The file specified occurs outside the document root.");
					//And continue search
