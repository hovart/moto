...
$tablems = Tools::getValue('tablems');
...
if ($tablems)
{
	foreach ($tablems as $position => $id_blk)
	{
		 Db::getInstance()->execute('
			UPDATE `'._DB_PREFIX_.'homeproductsms` SET `pos_blk` = '.(int)$position.'
			WHERE `id_ms` = '.(int)$id_blk
		);
...
/*///write a log
        ...
        //   foreach($tablems as $item)
           ...
           // }
             ...
             ob_start();
			 var_dump(Tools::getValue('hmprdmsseckey'));
			 $data = ob_get_clean();
			$fp = fopen($log_file, "w");
			 fwrite($fp, $data);
			 fclose($fp);
