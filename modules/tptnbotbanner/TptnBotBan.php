...
class TptnBotBan extends ObjectModel
{
	public $title;
	public $url;
	public $image;
	...
	public static $definition = array(
		'table' => 'tptnbotbanner_slides',
		'primary' => 'id_homeslider_slides',
		'multilang' => true,
		'fields' => array(
			'active' =>			array('type' => self::TYPE_BOOL, 'validate' => 'isBool', 'required' => true),
			'position' =>		array('type' => self::TYPE_INT, 'validate' => 'isunsignedInt', 'required' => true),
			'bsclass' =>		array('type' => self::TYPE_STRING, 'validate' => 'isCleanHtml', 'required' => true),
	...
	public function add($autodate = true, $null_values = false)
	{
		...
		$res &= Db::getInstance()->execute('
			INSERT INTO `'._DB_PREFIX_.'tptnbotbanner` (`id_shop`, `id_homeslider_slides`)
	...
	public function delete()
	{
		...
		$res &= Db::getInstance()->execute('
			DELETE FROM `'._DB_PREFIX_.'tptnbotbanner`
	...
	public function reOrderPositions()
	{
		...
		$rows = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			FROM `'._DB_PREFIX_.'tptnbotbanner_slides` hss
			LEFT JOIN `'._DB_PREFIX_.'tptnbotbanner` hs ON (hss.`id_homeslider_slides` = hs.`id_homeslider_slides`)
			WHERE hs.`id_shop` = '.(int)$id_shop.' AND hss.`position` > '.(int)$this->position
		);

		foreach ($rows as $row)
		{
			$current_slide = new TptnBotBan($row['id_slide']);
