...
var defaultDataParser = function(data) {
	if ( typeof data == 'string' ) {
		...
		this._messages($jqPm.ui.multiselect.constante.MESSAGE_ERROR, $jqPm.ui.multiselect.locale.errorDataFormat);
		data = false;
	}
	return data;
};
...
$jqPm.widget("ui.multiselect", {
  options: {
	...
	_create: function() {
		...
		this.selectedContainer = $jqPm('<div class="selected"></div>').appendTo(this.container);
		this.availableContainer = $jqPm('<div class="available"></div>').appendTo(this.container);
		this.selectedActions = $jqPm('<div class="actions ui-widget-header ui-helper-clearfix"><span class="count">0 '+$jqPm.ui.multiselect.locale.itemsCount+'</span><a href="#" class="remove-all">'+$jqPm.ui.multiselect.locale.removeAll+'</a></div>').appendTo(this.selectedContainer);
		this.availableActions = $jqPm('<div class="actions ui-widget-header ui-helper-clearfix"><input type="text" class="search empty ui-widget-content ui-corner-all"/><a href="#" class="add-all">'+$jqPm.ui.multiselect.locale.addAll+'</a></div>').appendTo(this.availableContainer);
		this.selectedList = $jqPm('<ul class="selected connected-list"><li class="ui-helper-hidden-accessible"></li></ul>').bind('selectstart', function(){return false;}).appendTo(this.selectedContainer);
		this.availableList = $jqPm('<ul class="available connected-list"><li class="ui-helper-hidden-accessible"></li></ul>').bind('selectstart', function(){return false;}).appendTo(this.availableContainer);
		...
		this.availableActions.find('input').width(Math.max(this.availableActions.width() - this.availableActions.find('a.add-all').width() - 30, 250));
			...
			this.selectedList.sortable({
				...
				update: function(event, ui) {
					...
					that.selectedList.find('li').each(function() {
						if ($jqPm(this).data('optionLink'))
							$jqPm(this).data('optionLink').remove().appendTo(that.element);
					});
				},
				receive: function(event, ui) {
					...
					that.selectedList.children('.ui-draggable').each(function() {
						$jqPm(this).removeClass('ui-draggable');
						$jqPm(this).data('optionLink', ui.item.data('optionLink'));
						$jqPm(this).data('idx', ui.item.data('idx'));
						that._applyItemState($jqPm(this), true);
					});
    ...
    **************************************/
	...
	_prepareLists: function(side, otherSide, opts) {
		...
		list.data('multiselect.sortable', opts[side].sortable )
	...
	_populateLists: function(options) {
		...
		var that = this;
		var items = $jqPm(options.map(function(i) {
