			...
			{if $language.iso_code == $lang_iso}
				<div class="current">
					<span>
						...
						{$language.iso_code|regex_replace:"/\s\(.*\)$/":""}</span>
				</div>
			{/if}
		{/foreach}
		<ul id="first-languages" class="languages-block_ul toogle_content">
			{foreach from=$languages key=k item=language name="languages"}
				...
				{else}	
					<span>
				{/if}
				{*<img src="{$img_lang_dir}{$language.id_lang}.jpg" alt="{$language.iso_code}" width="16" height="11" />*}{$language.name|regex_replace:"/\s\(.*\)$/":""}
				{if $language.iso_code != $lang_iso}
					</a>
				{else}
					</span>
