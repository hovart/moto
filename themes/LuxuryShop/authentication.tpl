...
{if !isset($email_create)}
	...
	<div class="row">
		<div class="col-xs-12 col-md-6">
			<form action="{$link->getPageLink('authentication', true)|escape:'html':'UTF-8'}" method="post" id="create-account_form" class="box">
				<h3 class="subheading">{l s='Create an account'}</h3>
				<div class="form_content clearfix">
					<p style="margin-bottom:15px;">{l s='Please enter your email address to create an account.'}</p>
	...
	{if isset($inOrderProcess) && $inOrderProcess && $PS_GUEST_CHECKOUT_ENABLED}
		<form action="{$link->getPageLink('authentication', true, NULL, "back=$back")|escape:'html':'UTF-8'}" method="post" id="new_account_form" class="box">
				
				<div id="opc_account_form" style="display: block; ">
					<h3 class="subheading bottom-indent">{l s='Instant checkout'}</h3>
					<!-- Account -->
					...
					{/if}
					<h3 class="subheading bottom-indent top-indent">{l s='Delivery address'}</h3>
					...
					<div id="opc_invoice_address"  class="unvisible">
						...
						{assign var=postCodeExist value=false}
						{assign var=dniExist value=false}
						<h3 class="subheading top-indent">{l s='Invoice address'}</h3>
...
{else}
	...
	<form action="{$link->getPageLink('authentication', true)|escape:'html':'UTF-8'}" method="post" id="account-creation_form" class="std box">
		...
		<div class="account_creation">
			<h3 class="subheading">{l s='Your personal information'}</h3>
			<div class="form-group">
				<label>{l s='Title'}</label>
		...
		{if isset($PS_REGISTRATION_PROCESS_TYPE) && $PS_REGISTRATION_PROCESS_TYPE}
			<div class="account_creation">
				<h3 class="subheading top-indent">{l s='Your address'}</h3>
				{foreach from=$dlv_all_fields item=field_name}
					{if $field_name eq "company"}
						{if !$b2b_enable}
							<p class="form-group">
			...
			<div class="account_creation dni">
				<h3 class="subheading">{l s='Tax identification'}</h3>
				<p class="required form-group">
					<label for="dni">{l s='Identification number'} <sup>*</sup></label>
		...
		<div class="submit clearfix">
			...
			<button type="submit" name="submitAccount" id="submitAccount" class="button button-medium">
				{l s='Register'}<i class="fa fa-chevron-right right"></i>
			</button>
