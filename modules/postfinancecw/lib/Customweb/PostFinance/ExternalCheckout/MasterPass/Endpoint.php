...
require_once 'Customweb/PostFinance/ExternalCheckout/MasterPass/AddressRequestParameterBuilder.php';
require_once 'Customweb/PostFinance/ExternalCheckout/MasterPass/RedirectionRequestParameterBuilder.php';
require_once 'Customweb/PostFinance/ExternalCheckout/MasterPass/AuthorizeRequestParameterBuilder.php';
 ...
 */
class Customweb_PostFinance_ExternalCheckout_MasterPass_Endpoint extends Customweb_Payment_ExternalCheckout_AbstractCheckoutEndpoint {
	...
	public function redirectAction(Customweb_Core_Http_IRequest $request){
		...
		try {
			...
			$builder = new Customweb_PostFinance_ExternalCheckout_MasterPass_RedirectionRequestParameterBuilder($context, $this->container, $this->getSecurityTokenFromRequest($request));
	...
	public function updateContextAction(Customweb_Core_Http_IRequest $request){
		...
		try {
			...
			$builder = new Customweb_PostFinance_ExternalCheckout_MasterPass_AddressRequestParameterBuilder($context, $this->container, $parameters);
			$responseParameters = $this->sendMessageToRemoteHost($builder->build());
	...
	public function confirmationAction(Customweb_Core_Http_IRequest $request){
		...
		try {
			...
			$templateContext->addVariable('javascript', $this->getAjaxJavascript('.postfinance-masterpass-shipping-pane', '.postfinance-masterpass-confirmation-pane'));
	...
	private function authorizeTransaction(Customweb_Payment_ExternalCheckout_IContext $context, Customweb_PostFinance_Authorization_Transaction $transaction){
		$this->getTransactionHandler()->beginTransaction();
		try {
			$builder = new Customweb_PostFinance_ExternalCheckout_MasterPass_AuthorizeRequestParameterBuilder($context, $this->container);
	...
	private function getRemoteUrl(){
		return $this->container->getConfiguration()->getBaseEndPointUrl() . Customweb_PostFinance_IAdapter::URL_DIRECT_ORDER;
	}
	...
	protected function getPaymentMethodByTransaction(Customweb_PostFinance_Authorization_Transaction $transaction){
	...
	private function sendMessageToRemoteHost(array $parameters) {
		...
		$responseParameters = Customweb_PostFinance_Util::getXmlAttributes($response->getBody());
