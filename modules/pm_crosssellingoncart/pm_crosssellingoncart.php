...
class PM_CrossSellingOnCart extends Module {
	...
	const NB_ACCESSORIES = 3;
	const NB_CROSSSELLING = 3;
	public function __construct($prefixFieldsOptions = 'PM_CSOC') {
		...
		if ($this->_onBackOffice()) {
			...
			if (version_compare(_PS_VERSION_, '1.5.0.0', '>=')) {
				$catalog_link = $this->context->link->getAdminLink('AdminProducts');
			} else {
				$catalog_link = 'index.php?tab=AdminCatalog&token='.Tools::getAdminTokenLite('AdminCatalog');
			}
			$this->_fieldsOptions = array(
				$this->prefixFieldsOptions.'_GLOBAL_H2' => array('title' => $this->l('Global options'), 'type' => 'h2', 'disable' => true),
				$this->prefixFieldsOptions.'_TITLE_BLOC' => array('title' => $this->l('Title Block'),'desc' => '', 'type' => 'textLang'),
				$this->prefixFieldsOptions.'_IMAGE_SIZE' => array ('title' => $this->l('Product image size'), 'desc' => '', 'type' => 'select', 'list' => array (), 'identifier' => 'value', 'default' => 'medium' ),
				$this->prefixFieldsOptions.'_DISPLAY_IN_PRODUCT' => array('title' => $this->l('Display on product page ?'), 'desc' => '', 'cast' => 'intval', 'type' => 'bool', 'default' => false),
				$this->prefixFieldsOptions.'_DISPLAY_IN_CART' => array('title' => $this->l('Display on shopping cart page ?'), 'desc' => '', 'cast' => 'intval', 'type' => 'bool', 'default' => true),
				...
				$this->prefixFieldsOptions.'_NB_PRODUCT_H2' => array('title' => $this->l('Products quantity display'), 'type' => 'h2', 'disable' => true),
				$this->prefixFieldsOptions.'_NB_PRODUCT' => array('title' => $this->l('Desktop'), 'desc' => '', 'cast' => 'intval', 'type' => 'text','default'=> 3),
				$this->prefixFieldsOptions.'_NB_PRODUCT_TABLET' => array('title' => $this->l('Tablet'), 'desc' => '', 'cast' => 'intval', 'type' => 'text','default'=> 2),
				$this->prefixFieldsOptions.'_NB_PRODUCT_MOBILE' => array('title' => $this->l('Mobile'), 'desc' => '', 'cast' => 'intval', 'type' => 'text','default'=> 1),
				$this->prefixFieldsOptions.'_DISPLAY_H2' => array('title' => $this->l('Custom display'), 'type' => 'h2', 'disable' => true),
				$this->prefixFieldsOptions.'_DISPLAY_IMG' => array('title' => $this->l('Display image ?'), 'desc' => '', 'cast' => 'intval', 'type' => 'bool', 'default' => true),
				$this->prefixFieldsOptions.'_DISPLAY_TITLE' => array('title' => $this->l('Display title ?'), 'desc' => '', 'cast' => 'intval', 'type' => 'bool', 'default' => true),
				$this->prefixFieldsOptions.'_DISPLAY_PRICE' => array('title' => $this->l('Display price ?'), 'desc' => '', 'cast' => 'intval', 'type' => 'bool', 'default' => true),
				$this->prefixFieldsOptions.'_DISPLAY_BUTTON' => array('title' => $this->l('Display "add to cart" button ?'), 'desc' => '', 'cast' => 'intval', 'type' => 'bool', 'default' => true),
				$this->prefixFieldsOptions.'_DISPLAY_AVAILABILITY' => array('title' => $this->l('Display availability info ?'), 'desc' => '', 'cast' => 'intval', 'type' => 'bool', 'default' => true),
				$this->prefixFieldsOptions.'_ACCESSORIES_H2' => array('title' => $this->l('Accessories'), 'type' => 'h2', 'disable' => true, 'desc' => $this->l('You need to link products beetwen them into the product associations configuration to have them linked into the cross selling list.').' <a href="'.$catalog_link.'" target="_blank">'.$this->l('Go to catalog').'</a>'),
				$this->prefixFieldsOptions.'_ACCESSORIES' => array('title' => $this->l('Display accessories'), 'cast' => 'intval', 'type' => 'bool', 'default'=> true),
				$this->prefixFieldsOptions.'_NB_ACCESSORIES' => array('title' => $this->l('Maximum accessories to display'),'desc' => '', 'cast' => 'intval', 'type' => 'text','default'=> self::NB_ACCESSORIES,'bloc_id'=> 'NB_ACCESSORIES','script'=>'handleSwitch("ACCESSORIES", "'.$this->prefixFieldsOptions.'")'),
				$this->prefixFieldsOptions.'_CROSSSELLING_H2' => array('title' => $this->l('"Frequently Bought Together"'), 'type' => 'h2', 'disable' => true, 'desc' => $this->l('The "Frequently Bought Together" are products wich are often order in the same time of a product in the cart of the customer.')),
				$this->prefixFieldsOptions.'_CROSSSELLING' => array('title' => $this->l('Display "Frequently Bought Together"'), 'desc' => '', 'cast' => 'intval', 'type' => 'bool', 'default'=> true),
				$this->prefixFieldsOptions.'_NB_CROSSSELLING' => array('title' => $this->l('Maximum products to display'),'desc' => '', 'cast' => 'intval', 'type' => 'text','default'=> self::NB_CROSSSELLING,'bloc_id'=> 'NB_CROSSSELLING','script'=>'handleSwitch("CROSSSELLING", "'.$this->prefixFieldsOptions.'")'),
				$this->prefixFieldsOptions.'_CROSSSELLING_NB_DAYS' => array('title' => $this->l('Number of days ?'), 'desc' => $this->l('Min: 1 - Max: 200'), 'cast' => 'intval', 'type' => 'text', 'default' => 160, 'suffix' => $this->l('days'), 'bloc_id'=> 'CROSSSELLING_NB_DAYS'),
			);
			$image = PM_CrossSellingOnCart::getImageType(true);
			foreach ( $image as $type ) {
				$this->_fieldsOptions [$this->prefixFieldsOptions.'_IMAGE_SIZE'] ['list'] [] = $type;
			}
		}
	}
	public function install(){
		...
		if (!$this->registerHook('shoppingCart') && !$this->registerHook('header') && !$this->registerHook('productFooter')) return false;
		return true;
	}
	private function _installDefaultConfig() {
		foreach ( $this->_fieldsOptions as $key => $field ) {
			if (!isset($field ['disable']) || !$field ['disable']) {				
				$val = $field ['default'];
				if (version_compare(_PS_VERSION_, '1.6.0.0', '>=') && $key == $this->prefixFieldsOptions.'_IMAGE_SIZE') {
					$val = $field ['default'].'_default';
				}
				if (trim($val)) {
					if (is_array($val)) {
						$val = serialize($val);
	...
	private function _uninstallDefaultConfig() {
		foreach ($this->_fieldsOptions as $key => $field) Configuration::deleteByName($key);
		return true;
	}
	private function _checkIfModuleIsUpdate($updateDb = false, $displayConfirm = true) {
		if (!$this->isRegisteredInHook('displayHeader') && !$this->isRegisteredInHook('header'))
			$this->registerHook('header');
		if (!$this->isRegisteredInHook('productFooter'))
	...
	public function saveConfig() {
		if (Tools::getValue('submitAdvancedStyles') || Tools::getValue('submitOptions_'.$this->prefixFieldsOptions)) {
			if (Tools::getValue($this->prefixFieldsOptions.'_CROSSSELLING_NB_DAYS', 0) < 1 || Tools::getValue($this->prefixFieldsOptions.'_CROSSSELLING_NB_DAYS', 0) > 200) {
				$this->_html .= $this->displayError($this->l('Value must be beetwen 1 and 200'));
			} else {
				foreach ( $this->_fieldsOptions as $key => $field ) {
				...
				if (!Tools::getIsset('products') || empty($_POST['products'])) {
					Configuration::updateValue($this->prefixFieldsOptions.'_PRODUCT_SELECTION', serialize(array()));
				} else {
	...
	public function displayShareConfig($share_base_config_url) {
	...
	public function displayConfig() {
		...
		foreach ($this->_fieldsOptions as $key => $field) {
			...
			if (isset($field['desc'])) {
				if ($field ['type'] != 'h2') {
					$this->_html .= ' &nbsp; <span>' . $field ['desc'] . '</span>';	
				} else {
					if (version_compare(_PS_VERSION_, '1.6.0', '>=')) {
						$this->_html .= ' <div class="bootstrap"><div class="alert alert-info">' . $field ['desc'] . '</div></div>';	
					} else {
						$this->_html .= ' <div class="info">' . $field ['desc'] . '</div>';	
					}
				}
			}
			if ($field ['type'] != 'h2') {
		...
		if (version_compare(_PS_VERSION_, '1.6.0', '>=')) {
			$this->_html .= ' <div class="bootstrap"><div class="alert alert-info">' . $this->l('You can choose products wich will be shown even if there are not respond to previous settings.') . '</div></div>';	
		} else {
			$this->_html .= ' <div class="info">' . $this->l('You can choose products wich will be shown even if there are not respond to previous settings.') . '</div>';	
		}
	...
	public function displayAdvancedStyles() {
		if (!is_writable(dirname(__FILE__) . '/css') || !is_dir(dirname(__FILE__) . '/css')) {
	...
	public function includeAdminCssJs() {
		if (version_compare(_PS_VERSION_, '1.6.0.0', '>=')) {
	...
	public function _postProcess() {
		...
		$this->saveConfig();
		if (Tools::getIsset('orderProductSelection')) {
	...
	public static function getImageType($include_pixels_size = false) {
		$result = array ();
		...
		foreach ( $result as $k => $img ) {
			...
			$image [$k]['name'] = $img ['name'].($include_pixels_size ? ' ('.$img ['width'].'px * '.$img ['height'].' px)' : '');
		}
		return $image;
	}
	public function getContent() {
		if (version_compare(_PS_VERSION_, '1.5.0.0', '>=')) $this->_html .= '<div id="pm_backoffice_wrapper" class="pm_bo_ps_'.Tools::substr(str_replace('.', '', _PS_VERSION_), 0, 2).'">';
		$this->initClassVar();
	...
	public function getElementProducts($products,$id_lang,$getProductsProperties = true) {
		if (version_compare(_PS_VERSION_, '1.5.0.0', '>=')) {
			...
			$result = self::Db_ExecuteS('
				SELECT p.*, pa.`id_product_attribute`, pl.`description`, pl.`description_short`, pl.`available_now`, pl.`available_later`, pl.`link_rewrite`, pl.`meta_description`, pl.`meta_keywords`, pl.`meta_title`, pl.`name`, i.`id_image`, il.`legend`, m.`name` AS manufacturer_name, tl.`name` AS tax_name, t.`rate`, cl.`name` AS category_default, DATEDIFF(p.`date_add`, DATE_SUB("'.date('Y-m-d').' 00:00:00", INTERVAL '.(Validate::isUnsignedInt(Configuration::get('PS_NB_DAYS_NEW_PRODUCT')) ? Configuration::get('PS_NB_DAYS_NEW_PRODUCT') : 20).' DAY)) > 0 AS new, (p.`price` * IF(t.`rate`,((100 + (t.`rate`))/100),1)) AS orderprice
				FROM `'._DB_PREFIX_.'product` p
	...
	private function _parseOptions($defaultOptions = array(), $options = array()) {
		...
		if (isset($options['tips']) && !empty($options['tips'])) $options['tips'] = Tools::stripslashes(addcslashes($options['tips'], '"'));
	...
	private function _getAccessoriesLight($productList, $limit, $ignoreList = array()) {
		...
		if (version_compare(_PS_VERSION_, '1.5.0.0', '>=')) {
			...
			'.(self::_isFilledArray($ignoreList) ? 'AND `id_product_2` NOT IN ('.(implode(',', $ignoreList)).')' : '').'
			...
			LEFT JOIN `'._DB_PREFIX_.'product` p ON (p.`id_product` = a.`id_product_2`)
			LEFT JOIN `'._DB_PREFIX_.'category_product` cp ON (cp.`id_category` = p.`id_category_default` AND cp.`id_product` = a.`id_product_2`)
			LEFT JOIN `'._DB_PREFIX_.'category_group` ctg ON (ctg.`id_category` = cp.`id_category`)
			INNER JOIN `'._DB_PREFIX_.'customer_group` cg ON (cg.`id_group` = ctg.`id_group` AND cg.`id_customer` = '.(int)$cookie->id_customer.')
			WHERE `id_product_1` IN ('.(implode(',', $productList)).')
			'.(self::_isFilledArray($ignoreList) ? 'AND `id_product_2` NOT IN ('.(implode(',', $ignoreList)).')' : '').'
			AND ctg.id_group IS NOT NULL
			ORDER BY RAND()
	...
	private function _getCrossSellingLight($productList, $limit) {
		if (!self::_isFilledArray($productList)) {
			return false;
		}
		if (version_compare(_PS_VERSION_, '1.5.0.0', '>=')) {
			return self::Db_executeS('
				...
				LEFT JOIN `'._DB_PREFIX_.'category_group` ctg ON (ctg.`id_category` = cp.`id_category` AND ctg.`id_group` = '.(int)Group::getCurrent()->id.')
				WHERE od.product_id NOT IN ('.implode(',', $productList).') AND od.id_order IN (
					SELECT od.id_order FROM '._DB_PREFIX_.'orders o
					JOIN '._DB_PREFIX_.'order_detail od ON (od.id_order = o.id_order)
					WHERE o.valid = 1 AND od.product_id IN ('.implode(',', $productList).') AND TO_DAYS("'.date('Y-m-d').' 00:00:00") - TO_DAYS(o.date_add) <= 182
				)
			 	AND ctg.id_group IS NOT NULL 
				LIMIT '.(int)$limit);
		} else {
	...
	public function hookShoppingCart($params) {
		if (!Configuration::get($this->prefixFieldsOptions.'_DISPLAY_IN_CART') && !isset($params['id_product'])) {
			return false;
		}
		...
		$csoc_display = Configuration::getMultiple(array(
            $this->prefixFieldsOptions.'_DISPLAY_IMG',
            $this->prefixFieldsOptions.'_DISPLAY_TITLE',
            $this->prefixFieldsOptions.'_DISPLAY_PRICE',
            $this->prefixFieldsOptions.'_DISPLAY_BUTTON',
            $this->prefixFieldsOptions.'_DISPLAY_AVAILABILITY'
        ));
        if (!$csoc_display[$this->prefixFieldsOptions.'_DISPLAY_IMG'] 
        	&& !$csoc_display[$this->prefixFieldsOptions.'_DISPLAY_TITLE']
        	&& !$csoc_display[$this->prefixFieldsOptions.'_DISPLAY_PRICE']
        	&& !$csoc_display[$this->prefixFieldsOptions.'_DISPLAY_BUTTON']
        	&& !$csoc_display[$this->prefixFieldsOptions.'_DISPLAY_AVAILABILITY']
        )
			return false;
		...
		$nbProductSelectionTablet = Configuration::get($this->prefixFieldsOptions.'_NB_PRODUCT_TABLET');
		$nbProductSelectionMobile = Configuration::get($this->prefixFieldsOptions.'_NB_PRODUCT_MOBILE');
		$nbAccessories = Configuration::get($this->prefixFieldsOptions.'_NB_ACCESSORIES');
		if (!$nbAccessories)
			$nbAccessories = (int)self::NB_ACCESSORIES;
		$nbCrossSelling = Configuration::get($this->prefixFieldsOptions.'_NB_CROSSSELLING');
		if (!$nbCrossSelling)
			$nbCrossSelling = (int)self::NB_CROSSSELLING;
		...
		$postProductSelection = @array_filter(unserialize(Configuration::get($this->prefixFieldsOptions.'_PRODUCT_SELECTION')));
		$productSelection = array();
		foreach ($postProductSelection as $k => $id_product) {
			if (version_compare(_PS_VERSION_, '1.6.0.12', '>=')) {
				if (!Product::checkAccessStatic((int)$id_product, false)) {
		...
		if (Configuration::get($this->prefixFieldsOptions.'_ACCESSORIES')) {
			$productAccessories = $this->_getAccessoriesLight($productsOnCart, $nbAccessories, $productArray);
		...
		if ((isset($params['on_product_page']) && $params['on_product_page'])) {
			...
			$productAccessories = $this->_getAccessoriesLight($productArray, $nbAccessories);
			if (self::_isFilledArray($productAccessories)) {
				foreach ($productAccessories AS $product) {
					$productsAccessories[] = (int)$product['id_product'];
				}
				$postProductSelection = array_diff($postProductSelection, $productsAccessories);
			}
		}
	...
	public function hookProductFooter() {
		if (Configuration::get($this->prefixFieldsOptions.'_DISPLAY_IN_PRODUCT')) {
			return $this->hookShoppingCart(array('on_product_page' => true, 'id_product' => Tools::getValue('id_product')));
		}
	}
