 ...
 *      [/] La valeur d'un critère est dorénavant vérifiée par la règle de validation "isAnything" au lieu de "isGenericName"
 *      [/] La viariable de réécriture "ModuleRoutes" prend en charge jusqu'à 5 filtres au lieu des 4 jusqu'à présent
 *
...
class UkooCompat extends Module
{
    ...
    public static $ModuleRoutes = array(
        'module-ukoocompat-catalog-0' => array(
            'controller' => 'catalog',
            'rule' => 'compat/{type_rewrite}/{marque_rewrite}/{modele_rewrite}/{moteur_rewrite}/{annee_rewrite}/{id_search}-{filters1}-{filters2}-{filters3}-{filters4}-{filters5}',
            'keywords' => array(
                'id_search' => array(
                    'regexp' => '[0-9]+',
                    'param' => 'id_search'),
                ...
                'filters5' => array(
                    'regexp' => '[0-9]+',
                    'param' => 'filters5'),
                'annee_rewrite' => array('regexp' => '[_a-zA-Z0-9-\pL]*')),
            'params' => array(
                'fc' => 'module',
                'module' => 'ukoocompat')),
        ...
        'module-ukoocompat-catalog-5' => array(
            ...
            'params' => array(
                'fc' => 'module',
                'module' => 'ukoocompat')),
        'module-ukoocompat-listing-0' => array(
            'controller' => 'listing',
            'rule' => 'compat/c/{type_rewrite}/{marque_rewrite}/{modele_rewrite}/{moteur_rewrite}/{annee_rewrite}/{category_rewrite}/{id_search}-{filters1}-{filters2}-{filters3}-{filters4}-{filters5}-{id_category}',
            'keywords' => array(
                'id_search' => array(
                    'regexp' => '[0-9]+',
                    'param' => 'id_search'),
        ...
        'module-ukoocompat-listing-all-0' => array(
            'controller' => 'listing',
            'rule' => 'compat/c/{type_rewrite}/{marque_rewrite}/{modele_rewrite}/{moteur_rewrite}/{annee_rewrite}/{category_rewrite}/{id_search}-{filters1}-{filters2}-{filters3}-{filters4}-{filters5}',
            'keywords' => array(
                'id_search' => array(
                    'regexp' => '[0-9]+',
                    'param' => 'id_search'),
    ...
    public function __construct()
    {
        ...
        $this->allowed_hooks = array(
            ...
            7 => array(
                'id' => 99,
                'name' => $this->l('Top MGD').' (displayNavMGD)')
        );
    ...
    public function install()
    {
        if (!parent::install()
            ...
            // Custom for MGD
            || !$this->registerHook('displayNavMGD')
...
//                var_dump($this->isCached('search-block.tpl', $id_cache));
     ...
     * @return string
     */
    public function hookDisplayNavMGD($params)
    {
        $params['hookDisplay'] = 'displayNavMGD';
        return $this->hookDisplayLeftColumn($params);
    }
