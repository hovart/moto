    ...
    public static $definition = array(
        ...
        'fields' => array(
            ...
            'value' => array(
                ...
                'lang' => true,
                'required' => true,
                'validate' => 'isAnything', // [ukoo_guillaume] 2016-03-30 :: Modifié car la plupart des fournisseurs ont des noms de modèles ou de cylindrées avec des caractères étranges
                'size' => 120)));
...
//            p($restrictions);
                    ...
                    if (!empty($restrictions)) {
                                ...
                                $sql .= ' JOIN `'._DB_PREFIX_.'ukoocompat_compat_criterion` '.$prefix.'
                                    ON ('.$prefix.'.`id_ukoocompat_filter` = '.(int)$id_filter_bis.'
                                        ...
                                        AND (
                                            ...
                                            // [ukoo_guillaume] 2016-03-30 :: ne pas tenir compte des critères universels pour le filtrage (uniquement pour les résultats)
                        ...
                        $sql .= ') uccX
						ON (ucc.`id_ukoocompat_compat` = uccX.`id_ukoocompat_compat`'.
                            // [ukoo_guillaume] 2016-03-30 :: ne pas tenir compte des critères universels pour le filtrage (uniquement pour les résultats)
                            ' AND ucc.`id_ukoocompat_criterion` != 0'.
                            // end [ukoo_guillaume] 2016-03-30
                            ($id_filter != null ? '
...
//                    UkooCache::write($cache_path, $criteria);
                    ...
                    if ($order_by == 'position') {
                        ...
                        // [ukoo_guillaume] 2016-03-30 :: trie des résultats par position
                        // suite et fin de la requête
                        $sql .= ' WHERE 1
						) ORDER BY uc.`position` '.pSQL($order_way);
                        $criteria = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS($sql);
//        [ukoo_guillaume] 2016-01-14 :: changer le système de cache par le natif PS
//                        // on écrit le résultat dans un fichier de cache
//                        UkooCache::write($cache_path, $criteria);
