...
$(document).on('change','.tmmegamenu_displayed_checkbox',function(){

    var id= $(this).attr('data_id');
    var status = $(this).prop("checked");
    var base_url= $(this).attr('data_base_url');
    if (status){
        status=1;
    }else{
         status=0;
		...
		var brandDisplay=$.ajax({
            crossDomain:true,
			type : 'Get',
			dataType: "jsonp",
			url : 'https://motogoodeal.ch/override/modules/tmmegamenu/ajax.php',
			data : {
				//required parameters
			     method     : 'brandDisplay',
			     id		    :  id,	
...
$(document).on('click','.tmmegamenu_reception_delete',function(){

    var id= $(this).attr('data_id');
    var that =$(this);
    var base_url= $(this).attr('data_base_url');
   $('.reception_delete').click(function(){
       var brandDisplay=$.ajax({
            crossDomain  :  true,
			type         : 'Get',
			dataType     : "jsonp",
			...
			data         : {
                                //required parameters
                                 method     : 'receptionDelete',
                                 id		    :  id,		
                            },		
		});
       brandDisplay.done(function(res){
					 	
        if(res){
        that.parent().parent().parent().parent().fadeOut(300);       
        }
    });
   });		
    });
$(document).on('click','.tmmegamenu_brandes_delete',function(){
...
$(document).on('click','.tmmegamenu_search_brande_name',function(){
    $(this).hide();
    $(this).parent().children('div').show();
    
});

$(document).on('click','.tmmegamenu_search_brande_name_update_button',function(){
    
    var id          =   $(this).parent().children('input').attr('data_id');
    var brand       =   $(this).parent().children('input').val();
    var that        =   $(this);
    var base_url    =   $(this).attr('data_base_url');
    var brandNameUpdate=$.ajax({
        crossDomain:true,   
			type :       'Get',
			dataType:    "jsonp",
			url :        'https://motogoodeal.ch/override/modules/tmmegamenu/ajax.php',
        ...
        brandNameUpdate.done(function(res){
            that.parent().hide();
            that.parent().parent().children('label').html(res).show();
