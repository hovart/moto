/*
* Mellow : Ce fichier a été modifié pour le rendre compatible avec le module Infine Ajax Scroll
*
* Le fichier original vient de la version 1.8.9 de la navigation à facettes (livré avec PS de 1.5.0.17 à 1.5.4.1)
* et est identique à celui de la version 1.9.1 (livré avec PS1.4.9.0 et PS1.4.10.0)
*
* Vous pouvez donc utiliser directement ce fichier sur les versions de PS depuis 1.4.9.0 jusqu'à 1.5.4.1
*
* Pour cela Il suffit de copier ce fichier dans le dossier /theme/votre-theme/js/modules/blocklayered/
* et d'activer le module avec la navigation à facettes.
*
...
/**** 2eme SERIE DE MODIFS ****/
//=> Mellow : Modif gestion loader du blocklayered 
	if (!ajaxLoaderOn && !$('#product_list').hasClass('isscrolling')) {
		$('#product_list').prepend('<div id="theloader">'+$('#layered_ajax_loader').html()+'</div>');
...
//=> Mellow : Ajout retour a la premiere page si rechargement
	if (jQuery.ias && !$('#product_list').hasClass('isscrolling') && params_plus != '') {
		$.scrollTo(0);
		if (params_plus.indexOf("/page-") >= 0)
			params_plus = params_plus.split('/page-')[0];
	}
//<= FIN AJOUT
...
//=> Mellow : Ajout reinitialisation paramettres blocklayered
		$('#theloader').remove(); // supression loader du blocklayered
		$('#product_list').removeClass('isscrolling');
		if (jQuery.ias) $.ias_hidepage();
//<= FIN AJOUT 
...
//=> Mellow : Modif gestion historique
		if (jQuery.ias) {
			if (current_friendly_url.indexOf("/page-") >= 0 && $('#product_list').hasClass('nohashes')) {
				current_friendly_url = current_friendly_url.replace(/\/page-(\d+)/, '');
				lockLocationChecking = true;
			}
			if (current_friendly_url == '#')
				current_friendly_url = '#/';
			if (current_friendly_url.indexOf("/page-") == -1) $.ias_init_pages();
			if (window.location.hash.replace(/\/page-(\d+)/, '').replace(/^#\//, '').replace(/^#/, '') == current_friendly_url.replace(/\/page-(\d+)/, '').replace(/^#\//, '').replace(/^#/, '')) {
				window.location.replace(('' + window.location).split('#')[0] + current_friendly_url); 
				slideUp = false;
			} else {
...
//=> Mellow : Ajout fonction pour reinitialisation surveillance du scroll par le module IAS
		complete: function() {
			if (jQuery.ias)
				if(slideUp)
					$.scrollTo($('#product_list').offset().top-40, 400, function(){ $.ias_reset(); });
				else $.ias_reset();
		//if (jQuery.ias && $.isFunction($.ias_reset)) $.ias_reset();
		}
//<= FIN AJOUT
/**** FIN TOUTES MODIFS ****/
	});
