	...
	{if empty($aErrors)}
		<div id="{$sGsrModuleName|escape:'htmlall':'UTF-8'}" class="bootstrap">
			<div id="comment-form" class="review block">
				<h1 class="h1 title">{l s='Review for' mod='gsnippetsreviews'} "{$aProduct.name|escape:'htmlall':'UTF-8'}"</h1>
				<div class="product">
					<p class="description">
				...
				{if !empty($aRating.replyDisplay) && !empty($aRating.replyData.sComment)}
					<blockquote class="blockquote-reverse">
						<p>
							...
							{$aRating.replyData.sComment nofilter}&nbsp;<img src="{$sImgUrl|escape:'htmlall':'UTF-8'}quotes-close.png" width="{$smarty.const._GSR_IMG_QUOTE_WIDTH|intval}" height="{$smarty.const._GSR_IMG_QUOTE_HEIGHT|intval}" alt="{l s='review reply' mod='gsnippetsreviews'}" />
						</p>
				...
				{if empty($aRating.review.reportId) && !empty($aRating.review.status) && !empty($sReportUrl) && !empty($bDisplayReportAbuse)}
					...
					<span class="pull-right">
						<a class="fancybox.ajax btn-sm btn-primary" id="reportReview1" rel="nofollow" href="{$sReportUrl|escape:'htmlall':'UTF-8'}" title="{l s='report a review' mod='gsnippetsreviews'}">{l s='Report abuse' mod='gsnippetsreviews'}</a>
					</span>
				{/if}
