(function($) {
	...
	window['PostFinanceCwLineItemGrid'] = {
		decimalPlaces: 2,
		currencyCode: 'EUR',
		
		init: function() {
			this.decimalPlaces = parseFloat($("#postfinancecw-decimal-places").val());
			this.currencyCode = $("#postfinancecw-currency-code").val();
			this.attachListeners();
		},
		
		attachListeners: function() {
			$(".postfinancecw-line-item-grid input.line-item-quantity").each(function() {
				PostFinanceCwLineItemGrid.attachListener(this);
			});
			$(".postfinancecw-line-item-grid input.line-item-price-excluding").each(function() {
				PostFinanceCwLineItemGrid.attachListener(this);
			});
			$(".postfinancecw-line-item-grid input.line-item-price-including").each(function() {
				PostFinanceCwLineItemGrid.attachListener(this);
			});
		},
		...
		attachListener: function(element) {
			$(element).change(function() {
				PostFinanceCwLineItemGrid.recalculate(this);
			});
		...
		recalculate: function(eventElement) {
			...
			var row = $('.postfinancecw-line-item-grid tr[data-line-item-index="' + lineItemIndex + '"]');
	...
	$(document).ready(function() {
		window['PostFinanceCwLineItemGrid'].init();
	});
