	...
	<div id="layer_cart">
		<div class="clearfix">
			<div class="layer_cart_product">
				<span class="cross" title="{l s='Close window' mod='blockcart'}"><i class="fa fa-times"></i></span>

				<h2>
					<i class="fa fa-check left"></i>
					{if $lang_iso == fr }
						{l s='Produit ajouté au panier avec succès' mod='blockcart'}
					{else}
				...
				<div class="layer_cart_product_info">
					...
					<div>
						<strong class="dark">
							{if $lang_iso == fr }
								{l s='Quantité' mod='blockcart'}
							{else}
								{l s='tQuantity' mod='blockcart'}
							{/if}
							:</strong>
			...
			<div class="layer_cart_cart">
				<h3 class="subheading">
					...
					<span class="ajax_cart_product_txt_s {if $cart_qties < 2} unvisible{/if}">
						{if $lang_iso == fr }
							{l s='Il y a [1]%d[/1] produits dans votre panier.' mod='blockcart' sprintf=[$cart_qties] tags=['<span class="ajax_cart_quantity">']}
						{else}
					...
					<span class="ajax_cart_product_txt {if $cart_qties > 1} unvisible{/if}">
						{if $lang_iso == fr }
							{l s='Il y a un produit dans votre panier' mod='blockcart'}
						{else}
				...
				<div class="layer_cart_row">
					<strong class="dark">
						{if $lang_iso == fr }
							{l s='Total produits' mod='blockcart'} :
				...
				<div class="button-container">	
					<span class="continue button std-btn" title="{l s='Continue shopping' mod='blockcart'}">
						<i class="fa fa-chevron-left left"></i>
						{if $lang_iso == fr }
							{l s='Poursuivre mes achats' mod='blockcart'}
						{else}
							{l s='Continue shopping' mod='blockcart'}
						{/if}
					</span>
					<a class="button" href="{$link->getPageLink("$order_process", true)|escape:"html":"UTF-8"}" title="{l s='Proceed to checkout' mod='blockcart'}" rel="nofollow">
						{if $lang_iso == fr }
							{l s='Accéder au panier' mod='blockcart'}
						{else}
