	...
	<div class="row">
		...
		<div class="col-xs-12 col-sm-6">
			<div id="address_invoice_form" class="select form-group selector1"{if $cart->id_address_invoice == $cart->id_address_delivery} style="display: none;"{/if}>
				...
				{else}
					<a href="{$link->getPageLink('address', true, NULL, "back={$back_order_page}?step=1&select_address=1{if $back}&mod={$back}{/if}")|escape:'html':'UTF-8'}" title="{l s='Add'}" class="button">
						<i class="fa fa-plus left"></i>{l s='Add a new address'}
					</a>
				{/if}
			</div>
		</div>
...
{if !$opc}
			<p class="cart_navigation clearfix">
				...
				<a href="{$link->getPageLink($back_order_page, true, NULL, "{if $back}back={$back}{/if}")|escape:'html':'UTF-8'}" title="{l s='Previous'}" class="button std-btn">
					<i class="fa fa-chevron-left left"></i>
					{l s='Previous'}
				</a>

				<button type="submit" name="processAddress" class="button standard-checkout">
					{l s='Proceed to checkout'}<i class="fa fa-chevron-right right"></i>
				</button>
			</p>
		</form>
{else}
...
{capture}{l s='Your billing address' js=1}{/capture}
{addJsDefL name=titleInvoice}{$smarty.capture.default|@addcslashes:'\''}{/addJsDefL}
{capture}{l s='Your delivery address' js=1}{/capture}
{addJsDefL name=titleDelivery}{$smarty.capture.default|@addcslashes:'\''}{/addJsDefL}
{capture}<a href="{$smarty.capture.addressUrlAdd}" title="{l s='Update' js=1}"><i class="fa fa-edit"></i></a>{/capture}
