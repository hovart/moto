...
class BT_AdminCtrl extends BT_GsrBaseCtrl
{  
	/**
	...
	public function run($aRequest)
	{
		...
		switch (self::$sAction) {
			case 'display' :
				...
				BT_GsrModuleTools::updateConfiguration();
				...
				// set params
				$aParams['oJsTranslatedMsg'] = BT_GsrModuleTools::jsonEncode($GLOBALS[_GSR_MODULE_NAME . '_JS_MSG']);

				// use case - type not define => first page requested
				if (empty(self::$sType)) {
					// update module if necessary
					$aParams['aUpdateErrors'] = GSnippetsReviews::$oModule->updateModule();
				}

				break;
			case 'update'   :
				// include admin update object
				require_once(_GSR_PATH_LIB_ADMIN . 'admin-update_class.php');

				$oAdminType = BT_AdminUpdate::create();
				break;
			case 'delete'   :
				// include admin delete object
				require_once(_GSR_PATH_LIB_ADMIN . 'admin-delete_class.php');
