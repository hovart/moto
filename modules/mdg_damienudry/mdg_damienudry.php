...
class mdg_damienudry extends Module
{
	private $sql_1 = 'mdg_damienudry';
	private $sql_2 = 'mdg_damienudrylang';
	private $area;
...
/* =============================================================== */
	public function install()
	{
		...
		if (!parent::install()
			...
			|| !$this->registerHook('displayDamienUdry')
...
/* =============================================================== */
	...
	{
		require_once dirname(__file__).'/classes/AdmMdgForm.php';
	...
	public function displayForm()
	{
		...
		$output .= $mdgForm->addField(array('type'=>'select','label'=>$this->l('Display on'),'name'=>'setting[hook]','id'=>'hook','value'=>$fields_values['hook'],
					'options' => array(
						...
						array('label'=>$this->l('displayDamienUdry (not native)'), 'value'=>'displayDamienUdry'),
		...
		$output .= $mdgForm->addField(array('type'=>'warning','class'=>'hook','id'=>'hook_displayDamienUdry', 'value'=>'<p>'.$this->l('To use this hook you must add "<b>{$HOOK_DISPLAYDAMIENUDRY}</b>" on your theme').'</p>'));
...
/* =============================================================== */
	...
	public function hookdisplayDamienUdry($params)
	{
		$this->context->controller->addCSS($this->_path.'css/'.$this->name.'.css', 'all');
		$this->context->smarty->assign(array(
			'homeSize' => Image::getSize(Image::getSize('home')?'home':'home_default'),
			'areaS' => $this->getAreaS('displayDamienUdry'),
			'hook' => 'categoryProducts'
		));
		return $this->display(__FILE__, $this->name.'.tpl');
	}
