...
PIE.RendererBase = {
     ...
     */
    getShape: function( name, subElName, parent, group , bgAttachment) {
        ...
        if( !shape ) {
            ...
            s = shape.style;
            if(typeof(bgAttachment) != 'undefined' && bgAttachment == 'fixed')
            	s.position = 'fixed';
            else
            	s.position = 'absolute';
            s.left = s.top = 0;
...
PIE.BackgroundRenderer = PIE.RendererBase.newRenderer( {
    ...
    drawBgImages: function() {
        ...
        if( images ) {
            ...
            while( i-- ) {
                img = images[i];
                shape = this.getShape( 'bgImage' + i, 'fill', this.getBox(), 2,img.bgAttachment );
