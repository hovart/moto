...
abstract class Customweb_PostFinance_AbstractAdapter implements Customweb_PostFinance_IAdapter {
	...
	public function getPaymentMethodByTransaction(Customweb_PostFinance_Authorization_Transaction $transaction){
	...
	protected function setTransactionAuthorizationState(Customweb_PostFinance_Authorization_Transaction $transaction, $parameters){
		...
		if ($transaction->isAuthorized()) {
			...
			if (isset($authorizationParameters['ALIAS']) && $transaction->getTransactionContext()->getAlias() !== null) {
				...
				else if (!$this->getPaymentMethodByTransaction($transaction) instanceof Customweb_PostFinance_Method_DirectDebit_Server_Abstract) {
	...
	protected function handleExtendedFraud(Customweb_PostFinance_Authorization_Transaction $transaction, $parameters){
		if ($transaction->isAuthorized()) {
	...
	public function processNewStatus(Customweb_PostFinance_Authorization_Transaction $transaction, $parameters, $initial){
		switch ($parameters['STATUS']) {
			case Customweb_PostFinance_IAdapter::STATUS_AUTHORISED:
			...
			case Customweb_PostFinance_IAdapter::STATUS_AUTHORISATION_REFUSED:
			case Customweb_PostFinance_IAdapter::STATUS_PAYMENT_REFUSED:
			case Customweb_PostFinance_IAdapter::STATUS_CANCELED:
			case Customweb_PostFinance_IAdapter::STATUS_CANCELED_WAITING:
				$transaction->appendAuthorizationParameters(array(
					'STATUS' => $parameters['STATUS'] 
				));
