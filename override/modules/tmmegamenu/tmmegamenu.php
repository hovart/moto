...
require_once(dirname(__FILE__) . '/classes/MegaMenuuSearch.php');
...
class TmmegamenuOverride extends Tmmegamenu
{
    ...
    public function __construct()
    {
        ...
        $this->displayName = $this->l('TemplateMonster Mega Menu');
    ...
    public function getContent()
    {
        ...
        } else if (Tools::getIsset('updateItemStatus')) // update item status from table
        {
            ...
            if (!$check_banner_fields = $this->preUpdateBanner()) {
                if ($id_banner = $this->addBanner()) {
                    if (Tools::isSubmit('updateBannerStay'))
                        Tools::redirectAdmin($this->context->link->getAdminLink('AdminModules', true) .
                            '&configure=' . $this->name . '&tab_module=' . $this->tab . '&module_name=' . $this->name . '&editBanner&id_item=' . $id_banner);
                } else
            ...
            if (!$check_video_fields = $this->preUpdateVideo()) {
                if ($video_id = $this->addVideo()) {
                    if (Tools::isSubmit('updateVideoStay'))
                        Tools::redirectAdmin($this->context->link->getAdminLink('AdminModules', true) .
                            '&configure=' . $this->name . '&tab_module=' . $this->tab . '&module_name=' . $this->name . '&editVideo&id_item=' . $video_id);
                } else
        ...
        } else if (Tools::getIsset('deleteVideo')) {
            ...
            if (!$check_map_fields = $this->preUpdateMap()) {
                if ($map_id = $this->addMap()) {
                    if (Tools::isSubmit('updateMapStay'))
                        Tools::redirectAdmin($this->context->link->getAdminLink('AdminModules', true) .
                            '&configure=' . $this->name . '&tab_module=' . $this->tab . '&module_name=' . $this->name . '&editMap&id_item=' . $map_id);
                } else
        ...
        } else if (Tools::getIsset('deleteMap')) {
        ...
        if ((Tools::getIsset('addItem') || Tools::getIsset('editItem')) && !$check_item_fields)
            $output .= $this->display($this->local_path, 'views/templates/admin/additem.tpl');
        else if ((Tools::getIsset('addHtml') || Tools::getIsset('editHtml')) && !$check_html_fields)
            $output .= $this->renderAddHtml();
        else if ((Tools::getIsset('addLink') || Tools::getIsset('editLink')) && !$check_link_fields)
            $output .= $this->renderAddLink();
        else if ((Tools::getIsset('addBanner') || Tools::getIsset('editBanner')) && !$check_banner_fields)
            $output .= $this->renderAddBanner();
        else if ((Tools::getIsset('addVideo') || Tools::getIsset('editVideo')) && !$check_video_fields)
            $output .= $this->renderAddVideo();
        else if ((Tools::getIsset('addMap') || Tools::getIsset('editMap')) && !$check_map_fields)
            $output .= $this->renderAddMap();
        else {
            if (!$check_item_fields && !$check_html_fields && !$check_link_fields && !$check_banner_fields && !$check_video_fields && !$check_map_fields) {
    ...
    protected function setTemplateVariables()
    {
        ...
        $megamenusearch = new MegaMenuSearch();
        ...
        //search
        //lists
        $this->context->smarty->assign('brands', $megamenusearch->getBrandes());
        $this->context->smarty->assign('receptions', $megamenusearch->getReception());
        $this->context->smarty->assign('articles', $megamenusearch->getArticles());
        $this->context->smarty->assign('categories', $megamenusearch->getCategories());
        $this->context->smarty->assign('allBrands', $megamenusearch->getAllBrandes());
        //paginations
        $this->context->smarty->assign('marksPagination', $megamenusearch->marksPagination());
        $this->context->smarty->assign('receptionPagination', $megamenusearch->receptionPagination());
        $this->context->smarty->assign('articlePagination', $megamenusearch->ArticlesPagination());
        $this->context->smarty->assign('categoryPagination', $megamenusearch->categoryPagination());
    ...
    protected function preUpdateBanner()
    {
        ...
        if (Tools::isEmpty(Tools::getValue('image_' . $this->default_language['id_lang'])))
    ...
    protected function addMap()
    {
        ...
        if (isset($_FILES['marker']) && isset($_FILES['marker']['tmp_name']) && !empty($_FILES['marker']['tmp_name'])) {
            ...
            elseif (!($tmp_name = tempnam(_PS_TMP_IMG_DIR_, 'PS')) || !move_uploaded_file($_FILES['marker']['tmp_name'], $tmp_name))
                return false;
            elseif (!ImageManager::resize($tmp_name, dirname(__FILE__) . '/img/markers/marker-' . $random_name . '-' . (int)$map->id_shop . '.jpg', 64, 64))
                $errors[] = $this->displayError($this->l('An error occurred during the image upload process.'));
            unlink($tmp_name);
    ...
    public function getMarks(){
       ...
       $id_lang = $cookie->id_lang;
        $marks = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
                SELECT *   FROM `ps_marks` WHERE status = 1  ORDER BY brand ASC');
          if($id_lang == 1){
             $options = '<option value="" >Choisissez une marque</option>';
             }
             elseif ($id_lang == 2) {
                 $options = '<option value="" >Marke wählen</option>';
             }
        foreach ($marks as $mark) {
            if($id_lang == 1){

                $options.="<option value='".$mark['brand']."'>".$mark['brand']."</option>";
            }else{
                $options.="<option value='".$mark['brand']."'>".$mark['brand']."</option>";
            }
        }
            ...
            foreach ($top_items as $key => $top) {
                ...
                if (!$top['is_mega']) {
                    ...
                    if ($rows = $megamenu->getMegamenuRow((int)$top['id_item'])) {
                        ...
                        foreach ($rows as $row) {
                            if ($top['unique_code'] == "it_87565369") {
                                ...
                                <div class="row">
                                    <div class="col-xs-12 tmmegamenuheaderfr" >

                                    </div>
                                </div>
                                <div class=" row">
                                
                                         <form action="'.Tools::getHttpHost(true).__PS_BASE_URI__.$this->context->language->iso_code.'/searchResult"  id="tmmegamenuSearchForm" method="get" >
                                             <div class="megamenu-col megamenu-col-1-1 col-sm-12">
                                               
                                                 
                                                 
                                                      <div class="row search_container">
                                                        <div class="col-xs-12 col-sm-3">
                                                            <label class="reception_by_type_title button_disable " id="reception_by_type_title"   data-token=data-token="{$token}">' .  $this->l("Réception par type") . '</label>   
                                                        </div>
                                                        <div class="col-xs-10 col-sm-7 reception_by_type_input_container">
                                                            <input style=   class="  reception_by_type button_disable "  id="reception_by_type_input" type="text" name="reception_by_type"  /> 
                                                        </div>
                                                        <div class="col-xs-2 col-sm-2 reception_by_type_button_container">
                                                            <button  class=" reception_by_type  " id="menuFilterButton"  disabled   >' . $this->l("Filtrer") . '</button>
                                                        </div>
                                                      
                                                      </div>
                                                      <div class="reception_by_type_filter row">
                                                     <div class=" col-sm-3  " >
                                                     <label for="marks" class="col-xs-12" >' . $this->l("Marque") . '</label>
                                                     <select name="marks" class="col-xs-12 reception_by_type"  id="marks">
                                                     '.$this->getMarks().'
                                                     ...
                                                     <div class=" col-sm-3   ">
                                                     <label for="cylinder" class="col-xs-12" >' . $this->l("Cylindrée ") . '</label>
                                                     <select name="cylinder" class="col-xs-12 reception_by_type"  id="cylinder" disabled>
                                                     ...
                                                     <div class=" col-sm-3  ">
                                                     <label for="model" class="col-xs-12" >' . $this->l("Modèle") . '</label>
                                                     <select name="model" class="col-xs-12 reception_by_type"  id="model"  disabled>
                                                 
                                                     </select>
                                                     </div>

                                                     <div class=" col-sm-3 ">
                                                     <label for="year" class="col-xs-12" >' . $this->l("Année") . '</label>
                                                     <select name="year" class="col-xs-12 reception_by_type"  id="year"  disabled>
                                                     </select>
                                                     </div>
                                                     </div>
                                        ...
                                        </div> <hr class="afterSearch">';
                                }
                                elseif($this->context->language->id==2){
                                            
                                $this->menu .= '
                                             <div class="row">
                                        <div class="col-xs-12 tmmegamenuheaderge" >
                                         ...
                                         <form action="'.Tools::getHttpHost(true).__PS_BASE_URI__.$this->context->language->iso_code.'/searchResult" id="tmmegamenuSearchForm"  method="get" >
                                            <div class="megamenu-col megamenu-col-1-1 col-sm-12">
                                                      ...
                                                      <div class="row search_container">
                                                        <div class="col-xs-12 col-sm-3">
                                                            <label class="reception_by_type_title button_disable " id="reception_by_type_title"   data-token=data-token="{$token}">' .  $this->l("Typen scheinnummer") . '</label>   
                                                        </div>
                                                        <div class="col-xs-10 col-sm-7 reception_by_type_input_container">
                                                            <input style=   class="  reception_by_type button_disable "  id="reception_by_type_input" type="text" name="reception_by_type"  /> 
                                                        </div>
                                                      ...
                                                      <div class="reception_by_type_filter row">
                                                     <div class=" col-sm-3  " >
                                                     <label for="marks" class="col-xs-12" >' . $this->l("Marks") . '</label>
                                                     <select name="marks" class="col-xs-12 reception_by_type"  id="marks">
                                                     '.$this->getMarks().'
                                                     ...
                                                     <div class=" col-sm-3   ">
                                                     <label for="cylinder" class="col-xs-12" >' . $this->l("Hubraum ") . '</label>
                                                     <select name="cylinder" class="col-xs-12 reception_by_type"  id="cylinder" disabled>
                                                     ...
                                                     <div class=" col-sm-3  ">
                                                     <label for="model" class="col-xs-12" >' . $this->l("Modell") . '</label>
                                                     <select name="model" class="col-xs-12 reception_by_type"  id="model"  disabled>
                                                     ...
                                                     <div class=" col-sm-3 ">
                                                     <label for="year" class="col-xs-12" >' . $this->l("Baujahr") . '</label>
                                                     <select name="year" class="col-xs-12 reception_by_type"  id="year"  disabled>
    ...
    private function getHtmlFieldsValues($id)
    {
        ...
        else if (Tools::getIsset('editHtml') && (int)Tools::getValue('id_item') > 0)
    ...
    private function getLinkFieldsValues($id)
    {
        ...
        else if (Tools::getIsset('editLink') && (int)Tools::getValue('id_item') > 0)
    ...
    private function getBannerFieldsValues($id = false)
    {
        ...
        else if (Tools::getIsset('editBanner') && (int)Tools::getValue('id_item') > 0)
        ...
        foreach ($languages as $lang) {
            $fields_values['title'][$lang['id_lang']] = Tools::getValue('name_' . (int)$lang['id_lang'],
            ...
            $fields_values['image'][$lang['id_lang']] = Tools::getValue('image_' . (int)$lang['id_lang'],
                $megamenubanner->image[$lang['id_lang']]);
            $fields_values['public_title'][$lang['id_lang']] = Tools::getValue('public_title_' . (int)$lang['id_lang'],
                $megamenubanner->public_title[$lang['id_lang']]);
            $fields_values['description'][$lang['id_lang']] = Tools::getValue('description_' . (int)$lang['id_lang'],
    ...
    private function getVideoFieldsValues($id)
    {
        ...
        else if (Tools::getIsset('editVideo') && (int)Tools::getValue('id_item') > 0)
    ...
    private function getMapFieldsValues($id)
    {
        ...
        else if (Tools::getIsset('editMap') && (int)Tools::getValue('id_item') > 0)
    ...
    public function hookCustomSearch()
    {
