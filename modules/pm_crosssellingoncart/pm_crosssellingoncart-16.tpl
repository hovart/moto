...
<div id="csoc-container" class="{if isset($on_product_page) && $on_product_page}page-product-box{/if} {$csoc_prefix|strtolower}">
	...
	<div id="{$csoc_prefix}" class="{if isset($on_product_page) && $on_product_page}bx-wrapper block products_block csoc-block{/if} clearfix">
		...
		<div class="product-container {if isset($on_product_page) && $on_product_page}product-box{/if} image-block">
			...
			<div class="{if isset($on_product_page) && $on_product_page}{else}right-block{/if}">
				...
				*}
				{if (!$PS_CATALOG_MODE && !empty($csoc_display["{$csoc_prefix}_AVAILABILITY"]) && ((isset($cartProduct.show_price) && $cartProduct.show_price) || (isset($cartProduct.available_for_order) && $cartProduct.available_for_order)))}
	...
	$csocjqPm(document).ready(function() {
		...
		if ($csocjqPm('body#product').size() > 0) {
			...
			$csocjqPm(document).on('click', '#PM_CSOC a.ajax_add_to_cart_button', function(e){
				e.preventDefault();
				var owl = $csocjqPm("#{$csoc_prefix}").data('owlCarousel');
