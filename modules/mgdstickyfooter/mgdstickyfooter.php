...
class MGDSTICKYFOOTER extends Module
{
	public function __construct()
	{
		$this->name = 'mgdstickyfooter';
		$this->description = 'Affiche un sticky footer dans les pages produits';
		$this->tab = 'front_office_features';
		$this->version = '1.1.1';
		$this->author = 'emotionLoop';
		$this->need_instance = 0;
		$this->ps_versions_compliancy = array('min' => '1.5', 'max' => '1.6');
		$this->monolanguage_content = 'Mgdstickyfooter_MONOLANGUAGE';
		$this->text_editor_content = 'Mgdstickyfooter_TEXTEDITOR';
		$this->content_wrapper = 'Mgdstickyfooter_CONTENTWRAPPER';
		$this->content_wrapper_class = 'Mgdstickyfooter_CONTENTWRAPPER_CLASS';
		$this->content_wrapper_id = 'Mgdstickyfooter_CONTENTWRAPPER_ID';
		$this->bootstrap = true;
		$this->_html = '';
		...
		$this->displayName = $this->l('mgdStickyFooter');
		$this->description = $this->l('Affiche un sticky footer dans les pages produits');
	...
	public function install()
	{
		...
		if (!parent::install() || !$this->registerHook(array('header', 'StickyFooter')) || !$this->registerHook(array('header', 'StickyFooter')) || !MGDSTICKYFOOTERModel::createTables())
			return false;
	...
	public function uninstall()
	{
		$this->_clearCache('template.tpl');
		if (!parent::uninstall()
			|| !MGDSTICKYFOOTERModel::DropTables())
		{
	...
	public function addFilesToTemplate($path = null)
	{
		...
		$files_data = MGDSTICKYFOOTERModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id );
	...
	{
		$content_query = MGDSTICKYFOOTERModel::getContent( $this->selected_store_id, $this->selected_language_id );
		$pre_content = '';
		$pos_content = '';
		...
		if (Tools::isSubmit('submit'.$this->name))
		{
				...
				&& (int)Configuration::get('PS_LANG_DEFAULT') == $this->selected_language_id ))
			{
				...
				if (Tools::getValue('content_text') !== false && $this->ignore_changes_content_changes == false)
					MGDSTICKYFOOTERModel::setContent( Tools::getValue('content_text'), $this->selected_store_id, $this->selected_language_id );

				//store the files to be used
				if (Tools::getValue('headerFiles') !== false)
					MGDSTICKYFOOTERModel::setFiles(
	...
	public function displayForm()
	{
		...
		$helper->fields_value['headerFiles[]'] = $this->processSelectFilesForMultiselect(
																MGDSTICKYFOOTERModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id )
																);
...
class MGDSTICKYFOOTERModel extends ObjectModel
{

	public static $definition = array(
		'table' => 'mgdstickyfooter',
		'primary' => 'file_id',
		'multishop' => true,
		'multilang' => true,
		'fields' => array(
	...
	public static function createTables()
	{
		...
		return ( MGDSTICKYFOOTERModel::createFilesTable()
				&& MGDSTICKYFOOTERModel::createContentTable());
	}
