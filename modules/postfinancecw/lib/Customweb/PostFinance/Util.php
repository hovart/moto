...
final class Customweb_PostFinance_Util {
	...
	public static function applyOrderSchema(Customweb_PostFinance_Configuration $config, $transactionId){
		...
		if (!empty($orderSchema)) {
			...
			if ($totalLength > 30) {
				...
				$orderSchema = Customweb_PostFinance_Util::substrUtf8($orderSchema, min($lengthToReduce, strlen($orderSchema)), 
						strlen($orderSchema));
			}
	...
	public static function applyOrderDescriptionSchema(Customweb_PostFinance_Configuration $config, $transactionId){
	...
	public static function sendRequest($url, $parameters){
		...
		if ($handler->getStatusCode() != 200) {
			throw new Exception(
					'PostFinance Server response is: ' . $handler->getStatusCode() . ' ' . $handler->getStatusMessage());
		}
	...
	public static function calculateHash($parameters, $transaction, Customweb_PostFinance_Configuration $configuration){
		...
		uasort($cleanParameters, array('Customweb_PostFinance_Util', 'sortParameters'));
