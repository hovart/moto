 ...
 */
class totsplashscreen extends Module {
	...
	public function getContent() {
		...
		if (version_compare(_PS_VERSION_, '1.4.9', '<')) {
			$smarty->template_dir = dirname(__FILE__).'/views/templates/back';
		}
		...
		if (Tools::isSubmit('TOTsubmitNewsletter')) {
			...
			elseif (isset($this->valid)) {
				...
				if (Configuration::get('NW_CONFIRMATION_EMAIL') AND $this->getValue('TOTaction') AND (int) ($this->getValue('TOTaction') == 0))
					Mail::Send((int) ($params['cookie']->id_lang), 'newsletter_conf', Mail::l('Newsletter confirmation'), array(), pSQL($this->getValue('TOTemail')), NULL, NULL, NULL, NULL, NULL, dirname(__FILE__).'/mails/');
