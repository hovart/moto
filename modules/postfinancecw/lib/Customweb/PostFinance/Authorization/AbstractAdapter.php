...
abstract class Customweb_PostFinance_Authorization_AbstractAdapter extends Customweb_PostFinance_AbstractAdapter {
	...
	protected function handleFailedTransaction(Customweb_PostFinance_Authorization_Transaction $transaction, $failedTransaction) {
		if ($failedTransaction !== null && $failedTransaction instanceof Customweb_PostFinance_Authorization_Transaction) {
	...
	public function authorize(Customweb_Payment_Authorization_ITransaction $transaction, array $parameters) {		
		...
		// In all other cases we have to send the authorization request to the remote side
			...
			try {
				$builder = new Customweb_PostFinance_Authorization_Hidden_AuthorizationParameterBuilder($transaction, $this->getContainer(), $parameters);
				...
				if($this->getPaymentMethodByTransaction($transaction) instanceof  Customweb_PostFinance_Method_DirectDebit_Server_Abstract && isset($parameters['CARDNO'])){
					$parameters['CARDNO'] = Customweb_PostFinance_Util::maskIban($parameters['CARDNO']);
				}
